...
Besides single-cluster relocations, it evaluates bounded compound relocations (default: up to 3 chained time moves), so a small temporary loss in step 1 can still be accepted when the total combined delta is better.
...
\begin{itemize}[leftmargin=*]
  ...
  \item for each round, a bounded set of high-pressure activities is probed and the best local-delta move among them is selected,
...
\end{itemize}
Terms that depend only on week-level session counts (e.g., same-course weekly concentration)
remain constant under within-week neighborhoods and are still tracked in exact global rescoring.
...
\subsection{Final Deterministic Polish}
After stochastic improvement, the implementation runs a short final deterministic polish from the best-known state.
This captures remaining obvious negative-delta group/week relocations that may be missed in probabilistic exploration near the time-budget limit.
