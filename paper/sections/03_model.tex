...
day/week availability, duration feasibility, and optional locks.
...
\[
N_{g,c,k,w}=\#\{a\in\mathcal{A}: g\in\mathcal{G}(a),\ \mathrm{course}(a)=c,\ \mathrm{kind}(a)=k,\ \mathrm{week}(a)=w\},
\]
for \(k\in\{\mathrm{LEC},\mathrm{TUT}\}\), and define
\[
P_{\mathrm{same}}=\sum_{g,c,k,w}\max(0,N_{g,c,k,w}-1).
\]
In the current strict CP model, weeks are fixed per activity; therefore this term is
mainly active in post-solve interactive edits where activities may be shifted across weeks.
