#!/usr/bin/env python3
"""Synthetic/adversarial gates for MUNI-FSPSX frontier v27.
...
BOOTSTRAP = CHAIN_ROOT / "planora-muni-fspsx-frontier-v27-bootstrap.py"
INLINE_TRUST = CHAIN_ROOT / "planora-muni-fspsx-frontier-v27-inline-trust-root.txt"
...
def load_windows_safe_supervisor_subset(path: Path):
    ...
    module = types.ModuleType("planora_muni_v27_supervisor_windows_static_tests")
...
if os.name == "nt":
    ...
    runner = load(RUNNER, "planora_muni_v27_runner_tests")
    supervisor = load(SUPERVISOR, "planora_muni_v27_supervisor_tests")
    bootstrap = load(BOOTSTRAP, "planora_muni_v27_bootstrap_tests")
...
def synthetic_runtime_source_tree(sources: dict[str, bytes]):
    with tempfile.TemporaryDirectory(dir="/tmp", prefix="muni-v27-cache-release-") as raw:
        root = Path(raw)
...
class V27ProcObservationRegressionTests(unittest.TestCase):
    @staticmethod
...
class StaticContractTests(unittest.TestCase):
    ...
    def test_v27_preserves_v26_caps_and_mode_separation(self) -> None:
        certificate = json.loads(
            (CHAIN_ROOT / "planora-muni-fspsx-frontier-v27-certificate.json").read_bytes()
        )
    ...
    def test_v27_preserves_v23_caps_and_mode_separation(self) -> None:
        certificate = json.loads(
            (CHAIN_ROOT / "planora-muni-fspsx-frontier-v27-certificate.json").read_bytes()
        )
        resource = certificate["resource_contract"]
    ...
    def test_v27_preserves_all_v18_resource_floors_and_limits(self) -> None:
    ...
    def test_current_quality_closure_and_certificate_paths_are_unambiguous(self) -> None:
        ...
        self.assertEqual(
            ...
            str(CHAIN_ROOT / "planora-muni-fspsx-frontier-v27-certificate.json"),
        )
...
class RuntimeClosureTests(unittest.TestCase):
    def test_cp_model_warning_hash_matches_record_and_runtime_manifest(self) -> None:
        ...
        freeze = json.loads(
            (
                CHAIN_ROOT
                / "planora-muni-fspsx-frontier-v27-freeze-manifest.json"
            ).read_bytes()
        )
        source_rows = [
    ...
    def test_real_sealed_runtime_imports_ortools_without_live_site_packages(self) -> None:
        if os.environ.get("PLANORA_MUNI_V27_SKIP_HEAVY") == "1":
        ...
        completed = subprocess.run(
            [
                str(PYTHON), "-I", "-S", "-B", "-X",
                "pycache_prefix=/tmp/muni-v27-probe-pyc",
    ...
    def test_manifest_seal_failure_closes_root_source_and_runtime_descriptors(self) -> None:
        ...
        with synthetic_runtime_source_tree(sources) as (
            ...
            def reject_manifest(name: str, raw: bytes):
                if name == "muni-v27-runtime-manifest":
...
"""
    ...
    def test_real_record_bundle_builds_and_replays_without_official_sources(self) -> None:
        ...
        runtime_fds: list[int] = []
        with tempfile.TemporaryDirectory(dir="/tmp", prefix="muni-v27-runtime-test-") as raw:
            root = Path(raw)
...
class SealedImportProbeTests(unittest.TestCase):
    ...
    def test_real_chain_reaches_probe_admission_without_opening_inputs(self) -> None:
        ...
        manifest_path = CHAIN_ROOT / "planora-muni-fspsx-frontier-v27-freeze-manifest.json"
...
class ArtifactSchemaTests(unittest.TestCase):
    ...
    def test_controlled_unknown_schema_is_exact(self) -> None:
        payload = {
            "schema": "planora.muni-fspsx.frontier-v27.controlled-unknown.v1",
...
def runtime_import_probe() -> None:
    ...
    capture_fds: list[int] = []
    runtime_fds: list[int] = []
    with tempfile.TemporaryDirectory(dir="/tmp", prefix="muni-v27-import-probe-") as raw:
        root_fd = os.open(raw, os.O_RDONLY | os.O_DIRECTORY)
        try:
...
if __name__ == "__main__":
    if len(sys.argv) == 2 and sys.argv[1] == "--runtime-import-probe":
        ...
        suite.addTests(loader.loadTestsFromTestCase(V27ProcObservationRegressionTests))
