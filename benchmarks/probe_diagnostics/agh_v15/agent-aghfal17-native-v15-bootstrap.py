#!/usr/bin/env python3
"""Capture and execute the AGH-FAL17 v15 launcher from sealed bytes."""
...
BOOTSTRAP_LOADER_PROTOCOL = "planora.aghfal17.native-v15-bootstrap-loader.v1"
...
tcb_path='/tmp/agent-aghfal17-native-v15-minimal-tcb.sha256'; tcb_expected='825b4b6656b67d706499095b184e55a0fe132310e7a92c7700634e8f0b26ffea'
root_ro=False
...
tcb_sealed=os.memfd_create('aghfal17-v15-minimal-tcb',getattr(os,'MFD_ALLOW_SEALING',0x0002)); os.write(tcb_sealed,tcb_raw); os.fchmod(tcb_sealed,0o400); fcntl.fcntl(tcb_sealed,fcntl.F_ADD_SEALS,required)
...
sealed=os.memfd_create('aghfal17-v15-bootstrap',getattr(os,'MFD_ALLOW_SEALING',0x0002)); view=memoryview(raw)
while view:
...
os.close(source_fd); os.close(parent_fd); filename=f'<sealed-aghfal17-v15-bootstrap:{actual}>'; sys.argv=[filename,*forwarded]
namespace={'__name__':'__main__','__file__':filename,'__package__':None,'__cached__':None,'__captured_sha256__':actual,'__captured_fd__':sealed,'__captured_source_path__':path,'__bootstrap_loader_protocol__':'planora.aghfal17.native-v15-bootstrap-loader.v1','__minimal_tcb_evidence__':{'fd':tcb_sealed,'sha256':tcb_expected,'file_count':len(tcb),'uid':65534,'gid':65534,'root_mount_read_only':True,'non_writable_ancestors':True,'admitted_before_bootstrap_exec':True}}
...
def capture_sealed_source(
    ...
    try:
        ...
        target_fd = os.memfd_create(
            f"aghfal17-v15-{label}", getattr(os, "MFD_ALLOW_SEALING", 0x0002)
        )
        view = memoryview(raw)
        while view:
...
def main() -> int:
    ...
    for descriptor in (bootstrap_fd, launcher_fd, bash_fd):
    ...
    environment = {
        ...
        "LC_ALL": "C.UTF-8",
        "TZ": "UTC",
        "AGHFAL_NATIVE_V15_BOOTSTRAP_SHA256": str(executed),
        "AGHFAL_NATIVE_V15_BOOTSTRAP_FD": str(bootstrap_fd),
        "AGHFAL_NATIVE_V15_LAUNCHER_SHA256": launcher["sha256"],
        "AGHFAL_NATIVE_V15_LAUNCHER_FD": str(launcher_fd),
        "AGHFAL_NATIVE_V15_BASH_SHA256": bash["sha256"],
        "AGHFAL_NATIVE_V15_BASH_FD": str(bash_fd),
