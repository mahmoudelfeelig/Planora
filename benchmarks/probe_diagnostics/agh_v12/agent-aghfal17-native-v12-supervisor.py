...
RUNNER = Path("/tmp/agent-aghfal17-native-v12-runner.py")
GENERIC_VALIDATOR = Path("/tmp/agent-aghfal17-native-v12-generic-validator.py")
STDLIB_MANIFEST = Path("/tmp/agent-aghfal17-native-v12-stdlib.sha256")
MINIMAL_TCB_MANIFEST = Path("/tmp/agent-aghfal17-native-v12-minimal-tcb.sha256")
OFFICIAL_INSTANCE = (
    ROOT
...
PLANORA_SOURCES = {
    ...
    "planora_itc2019_decomposed_quality": ROOT / "benchmarks/itc2019_decomposed_quality.py",
    "planora_itc2019_factorized": ROOT / "benchmarks/itc2019_factorized.py",
    "planora_itc2019_generalized_occurrences": ROOT / "benchmarks/itc2019_generalized_occurrences.py",
    "planora_itc2019_global_components": ROOT / "benchmarks/itc2019_global_components.py",
...
EXPECTED_RUNNER_SHA256 = "11801fd9cc5f853058f8e1c57a269efcae702237522c47c26a14f823d3e9044a"
EXPECTED_HASHES = {
    "official_instance": "bae3363ed68e895280cd33bc20686bf396932f532c2b197f7b863f4167437528",
    "generic_validator": "9864bcc2222f3ca676f5226b46c8d832bcaf5031d9e3e90e3e272ae1c2fea742",
    ...
    "planora_itc2019_decomposed": "3f4b92f91867cd1205f1702f36923b3c19cb8ad8d39b43d34a3b15e07f502e05",
...
CAPTURE_MANIFEST_ENV = "AGHFAL_NATIVE_V12_CAPTURE_MANIFEST"
OUTPUT_BINDING_ENV = "AGHFAL_NATIVE_V12_OUTPUT_BINDING"
RUNTIME_BUNDLE_ENV = "AGHFAL_NATIVE_V12_RUNTIME_BUNDLE"
EXTERNAL_LOADER_PROTOCOL = "planora.aghfal17.native-v12-supervisor-loader.v1"
RUNNER_LOADER_PROTOCOL = "planora.aghfal17.native-v12-runner-loader.v1"
REQUIRED_SEALS = (
    fcntl.F_SEAL_SEAL
    | fcntl.F_SEAL_SHRINK
    | fcntl.F_SEAL_GROW
...
runtime_binding=json.loads(os.environ["AGHFAL_NATIVE_V12_RUNTIME_BUNDLE"]); runtime_row=os.fstat(runtime_root_fd)
...
captures=json.loads(os.environ["AGHFAL_NATIVE_V12_CAPTURE_MANIFEST"]); stdlib_fd=int(captures["stdlib_manifest"]["fd"]); stdlib_row=os.fstat(stdlib_fd); stdlib_raw=os.pread(stdlib_fd,stdlib_row.st_size,0)
...
sys.dont_write_bytecode=True; filename=f"<sealed-aghfal17-native-v12-runner:{actual}>"; sys.argv=[filename,*forwarded]
namespace={"__name__":"__main__","__file__":filename,"__package__":None,"__cached__":None,"__captured_sha256__":actual,"__runner_loader_protocol__":"planora.aghfal17.native-v12-runner-loader.v1"}
...
sys.path.insert(0,f"/proc/self/fd/{runtime_root_fd}"); filename=f"<sealed-aghfal17-native-v12-generic:{actual}>"; sys.argv=[filename,*forwarded]
namespace={"__name__":"__main__","__file__":filename,"__package__":None,"__cached__":None,"__captured_sha256__":actual,"__generic_loader_protocol__":"planora.aghfal17.native-v12-generic-loader.v1"}
exec(compile(source,filename,"exec",dont_inherit=True),namespace)
'''
...
def launcher_attestation() -> dict[str, Any]:
    ...
    if (
        ...
        != "planora.aghfal17.native-v12-sealed-launcher-bootstrap.v1"
        or any(
            re.fullmatch(r"[0-9a-f]{64}", str(value.get(key, ""))) is None
            for key in ("bootstrap_sha256", "launcher_sha256", "bash_sha256")
        )
        or any(type(value.get(key)) is not int for key in ("bootstrap_fd", "launcher_fd", "bash_fd"))
...
def process_group_usage(process_group: int) -> tuple[int, int, tuple[int, ...]]:
    ...
    for entry in Path("/proc").iterdir():
        if not entry.name.isdigit() or _process_group_from_stat(entry / "stat") != process_group:
            continue
        try:
...
def generation_accounting_sample(
    ...
    requested.append(
        ("supervisor", supervisor_pid, proc_stat_identity(supervisor_pid))
    )
            ...
            unavailable.append(
                {
                    ...
                    "observed_identity": (
                        None if replayed is None else list(replayed)
                    ),
            ...
            unavailable.append(
                {
                    ...
                    "observed_identity_after_status": (
                        None
                        if replayed_after is None
                        else list(replayed_after)
                    ),
...
def _stream_capture(path: Path, expected: str, label: str) -> tuple[int, dict[str, Any]]:
    ...
    try:
        ...
        target_fd = os.memfd_create(f"aghfal17-v3-{label}", getattr(os, "MFD_ALLOW_SEALING", 0x0002))
        digest = sha256()
        offset = 0
...
def build_runtime_bundle(
    *, runtime_root_fd: int, captures: Mapping[str, Mapping[str, Any]]
    ...
    try:
        ...
        manifest = {
            "schema": "planora.aghfal17.native-v12-sealed-runtime.v1",
        ...
        binding = {
            "protocol": "planora.aghfal17.native-v12-sealed-runtime.v1",
            "root_fd": root_fd,
            "root_identity": list(root_identity),
        ...
        if post_capture_gate is not None:
            raise RuntimeError(
                f"runtime bundle resource gate: {post_capture_gate}"
            )
        summary = {
...
def _arm_child(parent_pid: int) -> None:
    ...
    os.setsid()
    resource.setrlimit(resource.RLIMIT_AS, (ADDRESS_SPACE_CAP_BYTES, ADDRESS_SPACE_CAP_BYTES))
...
def admit_spawned_process_group(
    ...
    if (
        generation["errors"]
        or admitted_members.get(child.pid) != leader_identity
    ):
...
def stop_group(
    ...
    result["untrusted_generation_pids"] = [
        pid for pid, _identity in untrusted_final
    ]
    return result
...
def planned_command(
    ...
    return [
        f"/proc/self/fd/{python_fd}", "-I", "-S", "-B", "-X", f"pycache_prefix={pycache_prefix}", "-c", RUNNER_FD_LOADER,
        str(runner_fd), EXPECTED_RUNNER_SHA256, str(runtime_root_fd),
        "--execute-completion", "--allow-official-input", "--allow-solver", "--allow-publication",
    ]
...
def _read_relative_regular(dirfd: int, name: str, *, maximum_bytes: int) -> tuple[bytes, dict[str, int]]:
    ...
    try:
        ...
        if _stable_identity(before) != _stable_identity(after) or _stable_identity(after) != _stable_identity(named):
            raise RuntimeError(f"child artifact {name} identity drift")
        return b"".join(chunks), {"device": int(after.st_dev), "inode": int(after.st_ino), "size": int(after.st_size), "mode": stat.S_IMODE(after.st_mode), "uid": int(after.st_uid)}
    finally:
        os.close(descriptor)
    ...
    try:
        ...
        while child.poll() is None:
            ...
            if generation["errors"]:
                ...
                break
            elapsed = max(0.0, SUPERVISOR_HARD_WALL_SECONDS - (hard_deadline - time.monotonic()))
            sample = host_sample()
            (
                whole_rss,
                whole_swap,
        ...
        if report_identity is not None:
            try:
                ...
                if (
                    ...
                    raise RuntimeError("generic validator report pathname was substituted")
...
def agh_child_acceptance(
    *,
    ...
    if child_exit_code == 2 and child.get("status") == "NO_RESULT":
        ...
        if (
            child.get("schema")
            != "planora.agh-fal17.native-v12-runner.v1"
    ...
    planora_claims_ok = (
        ...
        and all(expected_planora_hashes[name] == value for name, value in final_planora.items())
        ...
        and all(expected_planora_hashes[name] == value for name, value in generic_planora.items())
    )
    ...
    generic_claims_ok = (
        ...
        == "planora.agh-fal17.native-v12-isolated-generic-validator.v1"
    ...
    if (
        child.get("schema") != "planora.agh-fal17.native-v12-runner.v1"
        ...
        or child.get("competitor_schedule_or_result_used") is not False
        or child.get("competitor_placement_or_hint_used") is not False
        or report.get("schema") != "planora.agh-fal17.native-v12-report.v1"
        or report.get("status")
        != "NATIVE_LOCAL_VALIDATION_PASSED_GENERIC_PENDING"
        ...
        or publication.get("completion-report.json", {}).get("sha256")
        != report_digest
        or publication.get("completion-report.json", {}).get("size")
        != len(report_raw)
        or publication.get("completion-report.json", {}).get("publication_order")
        != 2
    ):
...
def _rename_noreplace(dirfd: int, source: str, destination: str) -> None:
    result = LIBC.renameat2(ctypes.c_int(dirfd), ctypes.c_char_p(os.fsencode(source)), ctypes.c_int(dirfd), ctypes.c_char_p(os.fsencode(destination)), ctypes.c_uint(RENAME_NOREPLACE))
    if result != 0:
        code = ctypes.get_errno()
        raise OSError(code, os.strerror(code), destination)


def publish_supervisor_report(*, dirfd: int, parent: Path, parent_identity: tuple[int, int, int, int], payload: Mapping[str, Any]) -> dict[str, Any]:
    raw = (json.dumps(payload, indent=2, sort_keys=True) + "\n").encode("utf-8")
    pending = f".supervisor-report.pending-{uuid.uuid4().hex}"
    descriptor = os.open(pending, os.O_RDWR | os.O_CREAT | os.O_EXCL | getattr(os, "O_NOFOLLOW", 0), 0o400, dir_fd=dirfd)
    ...
    try:
        ...
        if (int(named_parent.st_dev), int(named_parent.st_ino), stat.S_IMODE(named_parent.st_mode), int(named_parent.st_uid)) != parent_identity:
        ...
        exact, _ = _read_relative_regular(dirfd, "supervisor-report.json", maximum_bytes=64 << 20)
        if _stable_identity(named) != identity or exact != raw or _stable_identity(os.fstat(descriptor)) != identity:
            raise RuntimeError("supervisor report canonical final replay failed")
        return {"path": str(parent / "supervisor-report.json"), "sha256": sha256(exact).hexdigest(), "size": len(exact), "device": identity[0], "inode": identity[1]}
    except BaseException:
        if admitted is not None:
            try:
                current = os.stat("supervisor-report.json", dir_fd=dirfd, follow_symlinks=False)
...
def _private_directory(
    ...
    for _attempt in range(32):
        name = datetime.now(UTC).strftime(prefix + "-%Y%m%dT%H%M%SZ-") + uuid.uuid4().hex[:16]
        path = Path("/tmp") / name
        try:
            os.mkdir(path, 0o700)
        except FileExistsError:
...
def _private_run_directory() -> tuple[Path, int, tuple[int, int, int, int]]:
    return _private_directory("planora-aghfal17-native-v12-run")


def _private_runtime_directory() -> tuple[Path, int, tuple[int, int, int, int]]:
    return _private_directory("planora-aghfal17-native-v12-runtime")
...
def run_supervised() -> dict[str, Any]:
    ...
    try:
        ...
        scratch_dir, scratch_fd, scratch_identity = _private_directory(
            "planora-aghfal17-native-v12-scratch"
        )
        ...
        output_binding = {"fd": run_dir_fd, "path": str(run_dir), "device": run_dir_identity[0], "inode": run_dir_identity[1], "mode": run_dir_identity[2], "uid": run_dir_identity[3]}
        ...
        stdout_fd = os.open("child.stdout.log", os.O_WRONLY | os.O_CREAT | os.O_EXCL | getattr(os, "O_NOFOLLOW", 0), 0o400, dir_fd=run_dir_fd)
        stderr_fd = os.open("child.stderr.log", os.O_WRONLY | os.O_CREAT | os.O_EXCL | getattr(os, "O_NOFOLLOW", 0), 0o400, dir_fd=run_dir_fd)
        parent_pid = os.getpid()
        ...
        admitted_members: dict[int, tuple[int, int, int]] = {}
        try:
            child = subprocess.Popen(command, pass_fds=tuple(inherited), env=environment, stdin=subprocess.DEVNULL, stdout=stdout_fd, stderr=stderr_fd, close_fds=True, preexec_fn=lambda: _arm_child(parent_pid))
        except BaseException:
        ...
        if scratch_names or scratch_final_identity != scratch_identity:
            ...
            status = "FAILED"
        capture_end = {label: verify_sealed_capture(int(evidence["fd"]), evidence) for label, evidence in captures.items()}
        ...
        if (
            ...
            status = "FAILED"
        source_end = {label: verify_source_contract(evidence) for label, evidence in captures.items()}
        if verify_current_supervisor_contract() != supervisor_contract_start or supervisor_execution_sha256() != supervisor_start:
        ...
        payload = {
            "schema": "planora.agh-fal17.native-v12-supervisor.v1",
            "status": status,
            "errors": errors,
            "breach": breach,
        ...
        report_evidence = publish_supervisor_report(dirfd=run_dir_fd, parent=run_dir, parent_identity=run_dir_identity, payload=payload)
...
def sealed_import_probe_claim_predicates(
    ...
    is_object = stdout_classification == "object" and isinstance(
        child_payload, dict
    )
    ...
    return {
        ...
        == "planora.agh-fal17.native-v12-sealed-import-probe.v1",
        "status_pass": is_object and child_payload.get("status") == "PASS",
        "imported_modules_exact": is_object
        and imported_modules == expected_modules,
        "truth_schema_exact": is_object
        and validate_probe_truth_schema(child_payload),
        ...
        and child_payload.get("runner_sha256_start")
        == EXPECTED_RUNNER_SHA256,
        "runner_sha256_end_exact": is_object
        and child_payload.get("runner_sha256_end")
        == EXPECTED_RUNNER_SHA256,
...
def bounded_probe_rejection_diagnostics(
    *,
    ...
    predicate_results = {
        ...
        for name, accepted in acceptance_predicates.items()
    }
    return {
        "child_exit_code": child_exit_code,
        "failed_predicates": [
            name
            for name, accepted in predicate_results.items()
            if not accepted
        ],
    ...
    try:
        for label, path in PROBE_CAPTURE_SOURCES.items():
            descriptor, evidence = _stream_capture(
                path, _expected_hash(label), label
            )
            inherited.append(descriptor)
            captures[label] = evidence
        after_capture = host_sample()
        ...
        run_dir, run_dir_fd, run_identity = _private_directory(
            "planora-aghfal17-native-v12-import-probe"
        )
        inherited.append(run_dir_fd)
        scratch_dir, scratch_fd, scratch_identity = _private_directory(
            "planora-aghfal17-native-v12-import-probe-scratch"
        )
        inherited.append(scratch_fd)
        ...
        peak_supervisor_vmswap = 0
        peak_supervisor_in_group = False
        sampled_generation_identities: set[
        ...
        while child.poll() is None:
            ...
            elapsed = time.monotonic() - started
            sample = host_sample()
            accounting = generation_accounting_sample(
                os.getpid(), admitted_members
            )
            ...
            supervisor_vmswap = (
                ...
                else int(supervisor_row["vmswap_kib"])
            )
            supervisor_in_group = any(
                row["pid"] == os.getpid()
                and row["role"] == "generation_member"
                for row in accounting["sampled_processes"]
            )
        ...
        acceptance_predicates = {
            ...
            "process_group_cleanup_error_free": not group_cleanup.get(
                "errors", ()
            ),
        }
        errors: list[str] = []
        ...
        scratch_named = os.lstat(scratch_dir)
        private_scratch_contract = not scratch_entries and (
                int(scratch_named.st_dev),
                int(scratch_named.st_ino),
                stat.S_IMODE(scratch_named.st_mode),
                int(scratch_named.st_uid),
            ) == scratch_identity
        acceptance_predicates["private_scratch_contract"] = (
        ...
        acceptance_predicates["supervisor_contract_stable"] = (
            supervisor_contract_stable
        )
        ...
        if not launcher_bootstrap_stable:
            errors.append("sealed_launcher_bootstrap_drift")
        payload = {
            "schema": "planora.agh-fal17.native-v12-sealed-import-supervisor.v1",
        ...
        if errors:
            payload["rejection_diagnostics"] = (
                bounded_probe_rejection_diagnostics(
                    child_exit_code=exit_code,
        ...
        if child is not None and process_group is not None and not group_cleanup["empty"]:
            group_cleanup.update(
                stop_group(
                    child,
                    process_group,
...
def static_pins() -> dict[str, dict[str, Any]]:
    ...
    for label, path in CAPTURE_SOURCES.items():
        ...
        try:
            ...
            os.close(descriptor)
        actual = digest.hexdigest()
        if _stable_identity(before) != _stable_identity(after) or actual != _expected_hash(label):
            raise RuntimeError(f"static pin drift: {label}")
        rows[label] = {"path": str(path), "sha256": actual, "size": int(after.st_size), "identity": list(_stable_identity(after))}
    return rows


def self_test() -> dict[str, Any]:
    sample = {"mem_available_kib": 2_000_000, "swap_free_kib": 1_000_000, "pswpin_pages": 100, "pswpout_pages": 200}
    if breach_reason(elapsed=0, group_rss_kib=PROCESS_GROUP_MEMORY_LIMIT_KIB - 1, group_vmswap_kib=1, sample=sample, launch=False) != "process_group_vmrss_plus_vmswap_limit":
        raise AssertionError("combined process-group memory limit not enforced")
    if breach_reason(elapsed=0, group_rss_kib=PROCESS_GROUP_MEMORY_LIMIT_KIB - 1, group_vmswap_kib=0, sample=sample, launch=False) is not None:
        raise AssertionError("combined process-group memory exact lower boundary rejected")
    if breach_reason(
        elapsed=0,
    ...
    if breach_reason(elapsed=SUPERVISOR_HARD_WALL_SECONDS, group_rss_kib=0, group_vmswap_kib=0, sample=sample, launch=False) != "supervisor_hard_wall":
        raise AssertionError("hard wall not enforced")
    command = planned_command(
        16, 17, 18, Path("/tmp/nonexistent-private-pycache")
    )
    for flag in ("--execute-completion", "--allow-official-input", "--allow-solver", "--allow-publication"):
...
def main() -> int:
    if (
        globals().get("__external_loader_protocol__") != EXTERNAL_LOADER_PROTOCOL
        or globals().get("__external_expected_supervisor_sha256__") != globals().get("__captured_sha256__")
    ):
    ...
    if supervisor_execution_sha256() != start_hash or verify_current_supervisor_contract() != start_contract:
    ...
    return 0 if result.get("status") in {"PASS", "GO_FOR_INDEPENDENT_REVIEW", "COMPLETION_VALID"} else 2
