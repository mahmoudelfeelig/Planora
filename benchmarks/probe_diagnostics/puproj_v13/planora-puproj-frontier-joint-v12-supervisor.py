#!/usr/bin/env python3
"""Exact-byte whole-launch supervisor for PU-PROJ frontier-joint v12."""
...
GENERIC_VALIDATOR = Path("/tmp/planora-puproj-frontier-joint-v12-generic-validator.py")
...
EXPECTED_RUNNER_SHA256 = (
    "f647bf45168f392b9deb3274c498305a0cc892b8638af8535569dc2d0a03a6f2"
)
EXPECTED_HASHES = {
...
EXTERNAL_LOADER_PROTOCOL = "planora.puproj.frontier-v12-supervisor-loader.v1"
RUNNER_LOADER_PROTOCOL = "planora.puproj.frontier-v12-runner-loader.v1"
REQUIRED_SEALS = (
...
BOOTSTRAP_PYCACHE_PREFIX = Path(
    "/tmp/planora-puproj-frontier-joint-v12-bootstrap-pycache"
)
...
runtime_binding=json.loads(os.environ["PUPROJ_FRONTIER_V12_RUNTIME_BUNDLE"]); runtime_row=os.fstat(runtime_root_fd)
...
sys.dont_write_bytecode=True; filename=f"<sealed-puproj-frontier-v12-runner:{actual}>"; sys.argv=[filename,*forwarded]
namespace={"__name__":"__main__","__file__":filename,"__package__":None,"__cached__":None,"__captured_sha256__":actual,"__runner_loader_protocol__":"planora.puproj.frontier-v12-runner-loader.v1"}
...
def _stream_capture(
    ...
    try:
        ...
        target_fd = os.memfd_create(
            f"puproj-v12-{label}", getattr(os, "MFD_ALLOW_SEALING", 0x0002)
        )
        digest = sha256()
        offset = 0
...
def build_runtime_bundle(
    *, runtime_root_fd: int, captures: Mapping[str, Mapping[str, Any]],
    ...
    try:
        ...
        manifest = {
            "schema": "planora.puproj.frontier-v12-sealed-runtime.v1",
        ...
        binding = {
            "protocol": "planora.puproj.frontier-v12-sealed-runtime.v1",
            "root_fd": root_fd,
            "root_identity": list(root_identity),
...
def admit_probe_child_report(report: Any) -> dict[str, Any]:
    expected_keys = {
        "schema", "status", "elapsed_seconds", "executing_python",
        "runtime_bundle", "runtime_install", "loaded_runtime",
        "system_runtime_comparison", "imported_planora_modules",
        "imported_planora_module_count", "official_instance_opened",
        "checkpoint_or_incumbent_opened", "solver_execution_started",
        "solver_child_process_started", "probe_child_process_started",
        "solve_call_count", "official_solution_xml_published",
        "runner_sha256_start", "runner_sha256_end", "runner_hash_stable",
    }
    ...
    if (
        report.get("schema") != "planora.puproj.frontier-joint-v12-sealed-import-probe-child.v1"
    ...
    *,
    ...
    if child_exit_code == 3:
        ...
        if (
            ...
            or report.get("schema") != "planora.pu-proj.frontier-joint-v12.fresh-report.v1"
...
def publish_supervisor_report(
    *,
    ...
    descriptor = os.memfd_create(
        "planora-puproj-v12-supervisor-envelope",
...
def _private_run_directory() -> tuple[Path, int, tuple[int, int, int, int]]:
    return _private_directory("planora-puproj-frontier-v12-run")


def _private_runtime_directory() -> tuple[Path, int, tuple[int, int, int, int]]:
    return _private_directory("planora-puproj-frontier-v12-runtime")
...
def minimal_child_environment(
    *,
    ...
    pycache_prefix = str(Path(str(output_binding["path"])) / ".pycache-v12")
    return {
        "PATH": "/usr/bin:/bin",
        "LANG": "C.UTF-8",
        "LC_ALL": "C.UTF-8",
...
def run_supervised() -> dict[str, Any]:
    ...
    try:
        ...
        scratch_dir, scratch_fd, scratch_identity = _private_directory(
            "planora-puproj-frontier-v12-scratch"
        )
        ...
        pycache_prefix = run_dir / ".pycache-v12"
        ...
        os.close(stderr_fd)
        stderr_fd = -1
        status, errors, artifacts = child_acceptance_v12(
            dirfd=run_dir_fd,
            run_dir=run_dir,
            child_exit_code=child_exit_code,
        ...
        payload = {
            "schema": "planora.pu-proj.frontier-joint-v12-supervisor.v1",
            "status": status,
            "errors": errors,
            "breach": breach,
...
def sealed_import_probe() -> dict[str, Any]:
    ...
    if baseline["mem_available_kib"] < PROBE_INITIAL_MIN_MEM_AVAILABLE_KIB:
        return {
            "schema": "planora.puproj.frontier-joint-v12-sealed-import-probe.v1",
            "status": "NO_GO",
    ...
    try:
        ...
        inherited.append(run_dir_fd)
        scratch_dir, scratch_fd, scratch_identity = _private_directory(
            "planora-puproj-frontier-v12-probe-scratch"
        )
        inherited.append(scratch_fd)
        ...
        command = planned_command(
            ...
            runtime_root_fd, barrier_read_fd, run_dir / ".pycache-v12-probe",
        ...
        child_report = admit_probe_child_report(
            json.loads(stdout_raw.decode("utf-8")) if stdout_raw else {}
        )
        ...
        if (
            ...
            or child_report["runtime_bundle"].get("file_count") != runtime_replay.get("file_count")
            or child_report["runtime_bundle"].get("total_bytes") != runtime_replay.get("total_bytes")
        ):
...
def main() -> int:
    if globals().get(
        ...
        raise SystemExit("direct PU-PROJ v12 supervisor execution rejected")
