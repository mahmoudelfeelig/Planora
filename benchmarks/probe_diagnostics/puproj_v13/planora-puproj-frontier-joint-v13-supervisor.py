#!/usr/bin/env python3
"""Exact-byte whole-launch supervisor for PU-PROJ frontier-joint v13."""
...
ARTIFACT_ROOT = ROOT / "benchmarks/probe_diagnostics/puproj_v13"
SITE_PACKAGES = ROOT / ".venv/lib/python3.12/site-packages"
RUNNER = ARTIFACT_ROOT / "planora-puproj-frontier-joint-v13-runner.py"
FULL_INSTANCE = (
    ROOT
...
GENERIC_VALIDATOR = (
    ARTIFACT_ROOT / "planora-puproj-frontier-joint-v13-generic-validator.py"
)
...
EXPECTED_RUNNER_SHA256 = (
    "f639590511d2d4de5de9caf722bbc2cdb1fba692951cb38e580634cf24ff6e1b"
)
EXPECTED_HASHES = {
    ...
    "generic_validator": "7ca75fbe256c212130307d574ca1ef2592f46461b3eb33ef0ba2d5b4f8cfcc8c",
...
CAPTURE_MANIFEST_ENV = "PUPROJ_FRONTIER_V13_CAPTURE_MANIFEST"
OUTPUT_BINDING_ENV = "PUPROJ_FRONTIER_V13_OUTPUT_BINDING"
RUNTIME_BUNDLE_ENV = "PUPROJ_FRONTIER_V13_RUNTIME_BUNDLE"
PYCACHE_PREFIX_ENV = "PUPROJ_FRONTIER_V13_PYCACHE_PREFIX"
EXTERNAL_LOADER_PROTOCOL = "planora.puproj.frontier-v13-supervisor-loader.v1"
RUNNER_LOADER_PROTOCOL = "planora.puproj.frontier-v13-runner-loader.v1"
REQUIRED_SEALS = (
...
BOOTSTRAP_PYCACHE_PREFIX = Path(
    "/tmp/planora-puproj-frontier-joint-v13-bootstrap-pycache"
)
...
runtime_binding=json.loads(os.environ["PUPROJ_FRONTIER_V13_RUNTIME_BUNDLE"]); runtime_row=os.fstat(runtime_root_fd)
...
sys.dont_write_bytecode=True; filename=f"<sealed-puproj-frontier-v13-runner:{actual}>"; sys.argv=[filename,*forwarded]
namespace={"__name__":"__main__","__file__":filename,"__package__":None,"__cached__":None,"__captured_sha256__":actual,"__runner_loader_protocol__":"planora.puproj.frontier-v13-runner-loader.v1"}
...
def _stream_capture(
    ...
    try:
        ...
        target_fd = os.memfd_create(
            f"puproj-v13-{label}", getattr(os, "MFD_ALLOW_SEALING", 0x0002)
        )
        digest = sha256()
        offset = 0
...
def build_runtime_bundle(
    *, runtime_root_fd: int, captures: Mapping[str, Mapping[str, Any]],
    ...
    try:
        ...
        manifest = {
            "schema": "planora.puproj.frontier-v13-sealed-runtime.v1",
        ...
        binding = {
            "protocol": "planora.puproj.frontier-v13-sealed-runtime.v1",
            "root_fd": root_fd,
            "root_identity": list(root_identity),
...
def admit_probe_child_report(report: Any) -> dict[str, Any]:
    ...
    if (
        report.get("schema") != "planora.puproj.frontier-joint-v13-sealed-import-probe-child.v1"
...
def _probe_stream_diagnostics(
    ...
    *,
    ...
    return {
        "schema": "planora.puproj.frontier-joint-v13-child-stream-diagnostics.v1",
        "child_exit_code": child_exit_code,
...
def diagnose_probe_child_report(
    ...
    *,
    ...
    def rejected(classification: str, **details: Any):
        envelope = {
            "schema": "planora.puproj.frontier-joint-v13-child-rejection.v1",
    ...
    *,
    ...
    if child_exit_code == 3:
        ...
        if (
            ...
            or report.get("schema") != "planora.pu-proj.frontier-joint-v13.fresh-report.v1"
...
def publish_supervisor_report(
    *,
    ...
    descriptor = os.memfd_create(
        "planora-puproj-v13-supervisor-envelope",
...
def _private_run_directory() -> tuple[Path, int, tuple[int, int, int, int]]:
    return _private_directory("planora-puproj-frontier-v13-run")


def _private_runtime_directory() -> tuple[Path, int, tuple[int, int, int, int]]:
    return _private_directory("planora-puproj-frontier-v13-runtime")
...
def minimal_child_environment(
    *,
    ...
    pycache_prefix = str(Path(str(output_binding["path"])) / ".pycache-v13")
    return {
        "PATH": "/usr/bin:/bin",
        "LANG": "C.UTF-8",
        "LC_ALL": "C.UTF-8",
...
def run_supervised() -> dict[str, Any]:
    ...
    try:
        ...
        scratch_dir, scratch_fd, scratch_identity = _private_directory(
            "planora-puproj-frontier-v13-scratch"
        )
        ...
        pycache_prefix = run_dir / ".pycache-v13"
        ...
        os.close(stderr_fd)
        stderr_fd = -1
        status, errors, artifacts = child_acceptance_v13(
            dirfd=run_dir_fd,
            run_dir=run_dir,
            child_exit_code=child_exit_code,
        ...
        payload = {
            "schema": "planora.pu-proj.frontier-joint-v13-supervisor.v1",
            "status": status,
            "errors": errors,
            "breach": breach,
...
def sealed_import_probe() -> dict[str, Any]:
    ...
    if baseline["mem_available_kib"] < PROBE_INITIAL_MIN_MEM_AVAILABLE_KIB:
        return {
            "schema": "planora.puproj.frontier-joint-v13-sealed-import-probe.v1",
            "status": "NO_GO",
    ...
    try:
        ...
        inherited.append(run_dir_fd)
        scratch_dir, scratch_fd, scratch_identity = _private_directory(
            "planora-puproj-frontier-v13-probe-scratch"
        )
        inherited.append(scratch_fd)
        ...
        command = planned_command(
            ...
            runtime_root_fd, barrier_read_fd, run_dir / ".pycache-v13-probe",
...
def main() -> int:
    if globals().get(
        ...
        raise SystemExit("direct PU-PROJ v13 supervisor execution rejected")
