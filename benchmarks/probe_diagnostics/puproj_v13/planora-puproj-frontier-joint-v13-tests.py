#!/usr/bin/env python3
"""Lightweight static and adversarial gates for PU-PROJ v13."""
...
SUPERVISOR = ARTIFACT_ROOT / "planora-puproj-frontier-joint-v13-supervisor.py"
LAUNCHER = ARTIFACT_ROOT / "planora-puproj-frontier-joint-v13-launcher.py"
BOOTSTRAP = ARTIFACT_ROOT / "planora-puproj-frontier-joint-v13-bootstrap"
BOOTSTRAP_SOURCE = ARTIFACT_ROOT / "planora-puproj-frontier-joint-v13-bootstrap.c"
GENERIC = ARTIFACT_ROOT / "planora-puproj-frontier-joint-v13-generic-validator.py"
FREEZE = ARTIFACT_ROOT / "planora-puproj-frontier-joint-v13-freeze.json"
CERTIFICATE = (
    ARTIFACT_ROOT / "planora-puproj-frontier-joint-v13-completion-certificate.json"
)
...
runner = load(RUNNER, "puproj_v13_runner_tested")
guard = load(SUPERVISOR, "puproj_v13_supervisor_tested")
sealed_launcher = load(LAUNCHER, "puproj_v13_launcher_tested")
...
class AcceptanceTests(unittest.TestCase):
    ...
    def test_controlled_unknown_report_only(self) -> None:
        with tempfile.TemporaryDirectory(dir="/tmp") as raw:
            directory = Path(raw)
            report = {"schema": "planora.pu-proj.frontier-joint-v13.fresh-report.v1", "status": "CONTROLLED_UNKNOWN", "solver_input_mode": "OFFICIAL_INPUT_ONLY_FRESH", "checkpoint_or_incumbent_accessed": False, "admissible_as_solution": False}
            ...
            try:
                status, errors, _ = guard.child_acceptance_v13(dirfd=fd, run_dir=directory, child_exit_code=3, observed_child_elapsed_seconds=1.0)
            finally:
                os.close(fd)
    ...
    def test_complete_schema_and_report_last(self) -> None:
        with tempfile.TemporaryDirectory(dir="/tmp") as raw:
            directory = Path(raw)
            solution = b"<solution/>\n"
            report = {"schema": "planora.pu-proj.frontier-joint-v13.fresh-report.v1", "status": "COMPLETE_VALID", "solver_input_mode": "OFFICIAL_INPUT_ONLY_FRESH", "checkpoint_or_incumbent_accessed": False, "competitor_schedule_or_result_used": False, "competitor_placement_or_hint_used": False, "class_count": 8_813, "student_count": 38_437, "local_semantic_errors": [], "local_document_errors": [], "generic_validation": {"status": "COMPLETE_VALID", "classes": 8_813, "students": 38_437}}
            ...
            fd = os.open(directory, os.O_RDONLY | os.O_DIRECTORY)
            try:
                status, errors, _ = guard.child_acceptance_v13(dirfd=fd, run_dir=directory, child_exit_code=0, observed_child_elapsed_seconds=1.0)
            finally:
                os.close(fd)
...
class ProbeTests(unittest.TestCase):
    def _valid_probe_child_report(self) -> dict[str, object]:
        ...
        return {
            "schema": "planora.puproj.frontier-joint-v13-sealed-import-probe-child.v1",
            "status": "PASS",
            "elapsed_seconds": 1.0,
            "executing_python": {
            ...
            "loaded_runtime": {
                ...
                "python_executable_realpath": "/memfd:puproj-v13-python_binary (deleted)",
...
class TrustTests(unittest.TestCase):
    ...
    def test_v13_manifest_certificate_and_internal_pins_match_exact_bytes(self) -> None:
        freeze = json.loads(FREEZE.read_text(encoding="utf-8"))
        certificate = json.loads(CERTIFICATE.read_text(encoding="utf-8"))
        self.assertEqual(
            freeze["schema"],
            "planora.pu-proj.frontier-joint-v13-freeze.v1",
        )
        self.assertEqual(
            certificate["schema"],
            "planora.pu-proj.frontier-joint-v13.completion-certificate.v1",
        )
        self.assertEqual(
