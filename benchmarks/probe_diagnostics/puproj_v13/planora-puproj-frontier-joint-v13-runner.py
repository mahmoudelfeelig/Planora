...
def verify_system_python_provenance(*, phase: str) -> dict[str, Any]:
    ...
    expected_prefix_from_output = str(
        Path(str(output_binding.get("path"))) / ".pycache-v13"
    )
    if (
...
def load_capture_manifest(
    *, include_official: bool = True
    ...
    if (
        ...
        != "planora.puproj.frontier-v13-runner-loader.v1"
    ):
...
def verify_runtime_bundle(
    ...
    if binding.get("protocol") != "planora.puproj.frontier-v13-sealed-runtime.v1":
    ...
    if (
        manifest.get("schema") != "planora.puproj.frontier-v13-sealed-runtime.v1"
...
def _serialize_solution(problem, placements, student_classes) -> bytes:
    ...
    root = ElementTree.Element(
        "solution", {"name": problem.name, "technique": "planora-puproj-v13-fresh"}
    )
...
class L(importlib.abc.Loader):
 ...
 def exec_module(self,m):
  m.__file__="sealed:puproj-v13-validator:"+self.n; m.__captured_sha256__=self.h
...
stdlib=json.loads(sys.argv[7]); forwarded=sys.argv[8:]; sys.argv=["sealed:puproj-v13-generic-validator",*forwarded]
scope={"__name__":"__main__","__file__":"sealed:puproj-v13-generic-validator","__package__":None}
try: exec(compile(validator,"sealed:puproj-v13-generic-validator","exec"),scope)
...
def _generic_validation(
    ...
    solution_fd = os.memfd_create(
        "puproj-v13-generic-solution", getattr(os, "MFD_ALLOW_SEALING", 2)
    )
...
def run_frontier() -> tuple[int, dict[str, Any]]:
    ...
    base = {
        "schema": "planora.pu-proj.frontier-joint-v13.fresh-report.v1",
    ...
    if not result.is_feasible or stopped["signal"] is not None:
        ...
        return 3, {
            "schema": "planora.pu-proj.frontier-joint-v13-runner.v1",
    ...
    solution_fd = os.memfd_create("puproj-v13-local-solution", getattr(os, "MFD_ALLOW_SEALING", 2))
    try:
        os.write(solution_fd, xml_bytes)
        os.fchmod(solution_fd, 0o400)
        fcntl.fcntl(solution_fd, fcntl.F_ADD_SEALS, REQUIRED_SEALS)
...
def main() -> int:
    if not isinstance(globals().get("__captured_sha256__"), str):
        raise SystemExit("direct PU-PROJ v13 runner execution rejected")
