...
static const char *PYCACHE_PREFIX = "/tmp/planora-puproj-frontier-joint-v17-bootstrap-pycache";
...
"binding={'path':path,'sha256':expected,'fd':fd,'device':int(after.st_dev),'inode':int(after.st_ino),'size':int(after.st_size),'file_type':stat.S_IFMT(after.st_mode),'mode':stat.S_IMODE(after.st_mode),'uid':int(after.st_uid),'nlink':int(after.st_nlink),'seals':seals,'required_seals':required,'source_identity':list(claimed),'source_watch_fd':watch,'transport':'native_bootstrap_sealed_memfd_before_launcher_execution','bootstrap_sha256':os.environ['PUPROJ_V17_BOOTSTRAP_SHA256']}\n"
...
"filename=f'<sealed-puproj-frontier-v17-launcher:{expected}>'; sys.argv=[filename,*forwarded]\n"
"scope={'__name__':'__main__','__file__':filename,'__package__':None,'__cached__':None,'__captured_launcher_sha256__':expected,'__bootstrap_loader_protocol__':'planora.native-sealed-python-bootstrap.v1','__bootstrap_launcher_binding__':binding,'__bootstrap_manifest_binding__':manifest_binding,'__bootstrap_runtime_binding__':{'python_sha256':os.environ['PUPROJ_V17_PYTHON_SHA256'],'bootstrap_sha256':os.environ['PUPROJ_V17_BOOTSTRAP_SHA256'],'isolated':True,'no_site':True,'dont_write_bytecode':True,'pycache_prefix':sys.pycache_prefix},'__bootstrap_launcher_fd__':fd}\n"
...
static void die(const char *m){ fprintf(stderr,"PU-PROJ v17 native bootstrap: %s\n",m);exit(70); }
...
int main(int argc,char **argv){
  ...
  int sealed=memfd_new("planora-puproj-v17-launcher");if(sealed<0||write(sealed,raw,raw_n)!=(ssize_t)raw_n||fchmod(sealed,0400)||fcntl(sealed,F_ADD_SEALS,ALL_SEALS))die("launcher memfd sealing failed");
  ...
  if(clearenv()||setenv("PATH","/usr/bin:/bin",1)||setenv("LANG","C.UTF-8",1)||setenv("LC_ALL","C.UTF-8",1)||setenv("TZ","UTC",1)||setenv("PUPROJ_V17_BOOTSTRAP_SHA256",self_expected,1)||setenv("PUPROJ_V17_PYTHON_SHA256",PYTHON_SHA256,1))die("sanitized environment setup failed");
