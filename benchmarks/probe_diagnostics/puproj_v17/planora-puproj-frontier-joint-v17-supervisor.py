#!/usr/bin/env python3
"""Exact-byte whole-launch supervisor for PU-PROJ frontier-joint v17."""
...
GENERIC_VALIDATOR = (
    ARTIFACT_ROOT / "planora-puproj-frontier-joint-v17-generic-validator.py"
)
STDLIB_MANIFEST = (
    ARTIFACT_ROOT / "planora-puproj-frontier-joint-v17-stdlib.sha256"
)
...
EXPECTED_RUNNER_SHA256 = (
    "e2aee5251b73437b59b023a7de061420f90276baaed8d867e2ad9f9ef03b21f0"
)
EXPECTED_HASHES = {
...
EXTERNAL_LOADER_PROTOCOL = "planora.puproj.frontier-v17-supervisor-loader.v1"
RUNNER_LOADER_PROTOCOL = "planora.puproj.frontier-v17-runner-loader.v1"
REQUIRED_SEALS = (
...
BOOTSTRAP_PYCACHE_PREFIX = Path(
    "/tmp/planora-puproj-frontier-joint-v17-bootstrap-pycache"
)
...
runtime_binding=json.loads(os.environ["PUPROJ_FRONTIER_V17_RUNTIME_BUNDLE"]); runtime_row=os.fstat(runtime_root_fd)
...
sys.dont_write_bytecode=True; filename=f"<sealed-puproj-frontier-v17-runner:{actual}>"; sys.argv=[filename,*forwarded]
namespace={"__name__":"__main__","__file__":filename,"__package__":None,"__cached__":None,"__captured_sha256__":actual,"__runner_loader_protocol__":"planora.puproj.frontier-v17-runner-loader.v1"}
...
def _stream_capture(
    ...
    try:
        ...
        target_fd = os.memfd_create(
            f"puproj-v17-{label}", getattr(os, "MFD_ALLOW_SEALING", 0x0002)
        )
        digest = sha256()
        offset = 0
...
def build_runtime_bundle(
    *, runtime_root_fd: int, captures: Mapping[str, Mapping[str, Any]],
    ...
    try:
        ...
        manifest = {
            "schema": "planora.puproj.frontier-v17-sealed-runtime.v1",
        ...
        binding = {
            "protocol": "planora.puproj.frontier-v17-sealed-runtime.v1",
            "root_fd": root_fd,
            "root_identity": list(root_identity),
...
def admit_probe_child_report(report: Any) -> dict[str, Any]:
    ...
    if (
        report.get("schema") != "planora.puproj.frontier-joint-v17-sealed-import-probe-child.v1"
...
def _probe_stream_diagnostics(
    ...
    *,
    ...
    return {
        "schema": "planora.puproj.frontier-joint-v17-child-stream-diagnostics.v1",
        "child_exit_code": child_exit_code,
...
def diagnose_probe_child_report(
    ...
    *,
    ...
    def rejected(classification: str, **details: Any):
        envelope = {
            "schema": "planora.puproj.frontier-joint-v17-child-rejection.v1",
    ...
    *,
    ...
    if child_exit_code == 3:
        ...
        if (
            ...
            or report.get("schema") != "planora.pu-proj.frontier-joint-v17.fresh-report.v1"
...
def publish_supervisor_report(
    *,
    ...
    descriptor = os.memfd_create(
        "planora-puproj-v17-supervisor-envelope",
...
def _private_run_directory() -> tuple[Path, int, tuple[int, int, int, int]]:
    return _private_directory("planora-puproj-frontier-v17-run")


def _private_runtime_directory() -> tuple[Path, int, tuple[int, int, int, int]]:
    return _private_directory("planora-puproj-frontier-v17-runtime")
...
def minimal_child_environment(
    *,
    ...
    pycache_prefix = str(Path(str(output_binding["path"])) / ".pycache-v17")
    return {
        "PATH": "/usr/bin:/bin",
        "LANG": "C.UTF-8",
        "LC_ALL": "C.UTF-8",
...
def run_supervised() -> dict[str, Any]:
    ...
    try:
        ...
        scratch_dir, scratch_fd, scratch_identity = _private_directory(
            "planora-puproj-frontier-v17-scratch"
        )
        ...
        pycache_prefix = run_dir / ".pycache-v17"
        ...
        os.close(stderr_fd)
        stderr_fd = -1
        status, errors, artifacts = child_acceptance_v17(
            dirfd=run_dir_fd,
            run_dir=run_dir,
            child_exit_code=child_exit_code,
        ...
        payload = {
            "schema": "planora.pu-proj.frontier-joint-v17-supervisor.v1",
            "status": status,
            "errors": errors,
            "breach": breach,
...
def sealed_import_probe() -> dict[str, Any]:
    ...
    if baseline["mem_available_kib"] < PROBE_INITIAL_MIN_MEM_AVAILABLE_KIB:
        return {
            "schema": "planora.puproj.frontier-joint-v17-sealed-import-probe.v1",
            "status": "NO_GO",
    ...
    try:
        ...
        inherited.append(run_dir_fd)
        scratch_dir, scratch_fd, scratch_identity = _private_directory(
            "planora-puproj-frontier-v17-probe-scratch"
        )
        inherited.append(scratch_fd)
        ...
        command = planned_command(
            ...
            runtime_root_fd, barrier_read_fd, run_dir / ".pycache-v17",
...
def main() -> int:
    if globals().get(
        ...
        raise SystemExit("direct PU-PROJ v17 supervisor execution rejected")
