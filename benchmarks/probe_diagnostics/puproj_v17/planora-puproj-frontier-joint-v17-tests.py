#!/usr/bin/env python3
"""Lightweight static and adversarial gates for PU-PROJ v17."""
...
ARTIFACT_ROOT = ROOT / "benchmarks/probe_diagnostics/puproj_v17"
RUNNER = ARTIFACT_ROOT / "planora-puproj-frontier-joint-v17-runner.py"
SUPERVISOR = ARTIFACT_ROOT / "planora-puproj-frontier-joint-v17-supervisor.py"
LAUNCHER = ARTIFACT_ROOT / "planora-puproj-frontier-joint-v17-launcher.py"
BOOTSTRAP = ARTIFACT_ROOT / "planora-puproj-frontier-joint-v17-bootstrap"
BOOTSTRAP_SOURCE = ARTIFACT_ROOT / "planora-puproj-frontier-joint-v17-bootstrap.c"
GENERIC = ARTIFACT_ROOT / "planora-puproj-frontier-joint-v17-generic-validator.py"
STDLIB_MANIFEST = ARTIFACT_ROOT / "planora-puproj-frontier-joint-v17-stdlib.sha256"
FREEZE = ARTIFACT_ROOT / "planora-puproj-frontier-joint-v17-freeze.json"
CERTIFICATE = (
    ARTIFACT_ROOT / "planora-puproj-frontier-joint-v17-completion-certificate.json"
)
...
runner = load(RUNNER, "puproj_v17_runner_tested")
guard = load(SUPERVISOR, "puproj_v17_supervisor_tested")
sealed_launcher = load(LAUNCHER, "puproj_v17_launcher_tested")
...
class AcceptanceTests(unittest.TestCase):
    ...
    def test_controlled_unknown_report_only(self) -> None:
        with tempfile.TemporaryDirectory(dir="/tmp") as raw:
            directory = Path(raw)
            report = {"schema": "planora.pu-proj.frontier-joint-v17.fresh-report.v1", "status": "CONTROLLED_UNKNOWN", "solver_input_mode": "OFFICIAL_INPUT_ONLY_FRESH", "checkpoint_or_incumbent_accessed": False, "admissible_as_solution": False}
            ...
            try:
                status, errors, _ = guard.child_acceptance_v17(dirfd=fd, run_dir=directory, child_exit_code=3, observed_child_elapsed_seconds=1.0)
            finally:
                os.close(fd)
    ...
    def test_complete_schema_and_report_last(self) -> None:
        with tempfile.TemporaryDirectory(dir="/tmp") as raw:
            directory = Path(raw)
            solution = b"<solution/>\n"
            report = {"schema": "planora.pu-proj.frontier-joint-v17.fresh-report.v1", "status": "COMPLETE_VALID", "solver_input_mode": "OFFICIAL_INPUT_ONLY_FRESH", "checkpoint_or_incumbent_accessed": False, "competitor_schedule_or_result_used": False, "competitor_placement_or_hint_used": False, "class_count": 8_813, "student_count": 38_437, "local_semantic_errors": [], "local_document_errors": [], "generic_validation": {"status": "COMPLETE_VALID", "classes": 8_813, "students": 38_437}}
            ...
            fd = os.open(directory, os.O_RDONLY | os.O_DIRECTORY)
            try:
                status, errors, _ = guard.child_acceptance_v17(dirfd=fd, run_dir=directory, child_exit_code=0, observed_child_elapsed_seconds=1.0)
            finally:
                os.close(fd)
...
class ProbeTests(unittest.TestCase):
    def _valid_probe_child_report(self) -> dict[str, object]:
        ...
        return {
            ...
            "loaded_runtime": {
                ...
                "python_executable_realpath": "/memfd:puproj-v17-python_binary (deleted)",
    ...
    def test_probe_chain_cleanup_and_report_last_are_explicit(self) -> None:
        ...
        self.assertIn('run_dir / ".pycache-v17",', segment)
        self.assertNotIn(".pycache-v17-probe", segment)
    ...
    def test_probe_pycache_prefix_matches_environment_and_output_binding(self) -> None:
        output_path = Path("/tmp/planora-puproj-frontier-v17-test-output")
        output_binding = {
            "fd": 23,
        ...
        expected_prefix = output_path / ".pycache-v17"
...
class TrustTests(unittest.TestCase):
    ...
    def test_v17_manifest_certificate_and_internal_pins_match_exact_bytes(self) -> None:
        freeze = json.loads(FREEZE.read_text(encoding="utf-8"))
        certificate = json.loads(CERTIFICATE.read_text(encoding="utf-8"))
        self.assertEqual(
            freeze["schema"],
            "planora.pu-proj.frontier-joint-v17-freeze.v1",
        )
        self.assertEqual(
            certificate["schema"],
            "planora.pu-proj.frontier-joint-v17.completion-certificate.v1",
        )
        self.assertEqual(
