#!/usr/bin/env python3
"""Exact-byte process-group supervisor for AGH-FAL17 native v9."""
...
RUNNER = Path("/tmp/agent-aghfal17-native-v9-runner.py")
GENERIC_VALIDATOR = Path("/tmp/agent-aghfal17-native-v9-generic-validator.py")
STDLIB_MANIFEST = Path("/tmp/agent-aghfal17-native-v9-stdlib.sha256")
MINIMAL_TCB_MANIFEST = Path("/tmp/agent-aghfal17-native-v9-minimal-tcb.sha256")
OFFICIAL_INSTANCE = (
    ROOT
...
EXPECTED_RUNNER_SHA256 = "60b06dcb827279542f049a43723a2abd8381c50273e354c48c83f81868bb5236"
EXPECTED_HASHES = {
    "official_instance": "bae3363ed68e895280cd33bc20686bf396932f532c2b197f7b863f4167437528",
    "generic_validator": "80a04b60460f536db7e7198a611b713e048df789974e11520190f7f8c64825e7",
...
CAPTURE_MANIFEST_ENV = "AGHFAL_NATIVE_V9_CAPTURE_MANIFEST"
OUTPUT_BINDING_ENV = "AGHFAL_NATIVE_V9_OUTPUT_BINDING"
RUNTIME_BUNDLE_ENV = "AGHFAL_NATIVE_V9_RUNTIME_BUNDLE"
EXTERNAL_LOADER_PROTOCOL = "planora.aghfal17.native-v9-supervisor-loader.v1"
RUNNER_LOADER_PROTOCOL = "planora.aghfal17.native-v9-runner-loader.v1"
REQUIRED_SEALS = (
    fcntl.F_SEAL_SEAL
    | fcntl.F_SEAL_SHRINK
    | fcntl.F_SEAL_GROW
...
SUPERVISOR_HARD_WALL_SECONDS = 1_800.0
POLL_SECONDS = 0.10
...
runtime_binding=json.loads(os.environ["AGHFAL_NATIVE_V9_RUNTIME_BUNDLE"]); runtime_row=os.fstat(runtime_root_fd)
...
captures=json.loads(os.environ["AGHFAL_NATIVE_V9_CAPTURE_MANIFEST"]); stdlib_fd=int(captures["stdlib_manifest"]["fd"]); stdlib_row=os.fstat(stdlib_fd); stdlib_raw=os.pread(stdlib_fd,stdlib_row.st_size,0)
...
sys.dont_write_bytecode=True; filename=f"<sealed-aghfal17-native-v9-runner:{actual}>"; sys.argv=[filename,*forwarded]
namespace={"__name__":"__main__","__file__":filename,"__package__":None,"__cached__":None,"__captured_sha256__":actual,"__runner_loader_protocol__":"planora.aghfal17.native-v9-runner-loader.v1"}
...
sys.path.insert(0,f"/proc/self/fd/{runtime_root_fd}"); filename=f"<sealed-aghfal17-native-v9-generic:{actual}>"; sys.argv=[filename,*forwarded]
namespace={"__name__":"__main__","__file__":filename,"__package__":None,"__cached__":None,"__captured_sha256__":actual,"__generic_loader_protocol__":"planora.aghfal17.native-v9-generic-loader.v1"}
exec(compile(source,filename,"exec",dont_inherit=True),namespace)
'''
...
def build_runtime_bundle(
    *, runtime_root_fd: int, captures: Mapping[str, Mapping[str, Any]]
    ...
    try:
        ...
        manifest = {
            "schema": "planora.aghfal17.native-v9-sealed-runtime.v1",
        ...
        binding = {
            "protocol": "planora.aghfal17.native-v9-sealed-runtime.v1",
            "root_fd": root_fd,
            "root_identity": list(root_identity),
...
def agh_child_acceptance(
    *,
    ...
    if child_exit_code == 2 and child.get("status") == "NO_RESULT":
        ...
        if (
            child.get("schema")
            != "planora.agh-fal17.native-v9-runner.v1"
    ...
    generic_claims_ok = (
        ...
        == "planora.agh-fal17.native-v9-isolated-generic-validator.v1"
    ...
    if (
        child.get("schema") != "planora.agh-fal17.native-v9-runner.v1"
        ...
        or child.get("competitor_schedule_or_result_used") is not False
        or child.get("competitor_placement_or_hint_used") is not False
        or report.get("schema") != "planora.agh-fal17.native-v9-report.v1"
...
def _private_run_directory() -> tuple[Path, int, tuple[int, int, int, int]]:
    return _private_directory("planora-aghfal17-native-v9-run")


def _private_runtime_directory() -> tuple[Path, int, tuple[int, int, int, int]]:
    return _private_directory("planora-aghfal17-native-v9-runtime")
...
def run_supervised() -> dict[str, Any]:
    ...
    try:
        ...
        scratch_dir, scratch_fd, scratch_identity = _private_directory(
            "planora-aghfal17-native-v9-scratch"
        )
        ...
        payload = {
            "schema": "planora.agh-fal17.native-v9-supervisor.v1",
            "status": status,
            "errors": errors,
            "breach": breach,
...
def sealed_import_probe_claim_predicates(
    ...
    return {
        ...
        == "planora.agh-fal17.native-v9-sealed-import-probe.v1",
...
def run_sealed_import_probe() -> dict[str, Any]:
    ...
    try:
        ...
        run_dir, run_dir_fd, run_identity = _private_directory(
            "planora-aghfal17-native-v9-import-probe"
        )
        inherited.append(run_dir_fd)
        scratch_dir, scratch_fd, scratch_identity = _private_directory(
            "planora-aghfal17-native-v9-import-probe-scratch"
        )
        inherited.append(scratch_fd)
        ...
        if not launcher_bootstrap_stable:
            errors.append("sealed_launcher_bootstrap_drift")
        payload = {
            "schema": "planora.agh-fal17.native-v9-sealed-import-supervisor.v1",
...
def main() -> int:
    if (
        ...
        raise SystemExit("direct AGH-FAL17 native v9 supervisor execution rejected")
