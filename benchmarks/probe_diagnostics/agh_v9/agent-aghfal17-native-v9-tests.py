#!/usr/bin/env python3
"""Lightweight adversarial tests for the AGH-FAL17 native v9 diagnostic chain.
...
ARTIFACT_DIR = Path(os.environ.get("AGHFAL_NATIVE_V9_ARTIFACT_DIR", "/tmp"))
RUNNER_PATH = ARTIFACT_DIR / "agent-aghfal17-native-v9-runner.py"
SUPERVISOR_PATH = ARTIFACT_DIR / "agent-aghfal17-native-v9-supervisor.py"
GENERIC_PATH = ARTIFACT_DIR / "agent-aghfal17-native-v9-generic-validator.py"
LAUNCHER_PATH = ARTIFACT_DIR / "agent-aghfal17-native-v9-launcher.sh"
BOOTSTRAP_PATH = ARTIFACT_DIR / "agent-aghfal17-native-v9-bootstrap.py"
HARNESS_PATH = ARTIFACT_DIR / "agent-aghfal17-native-v9-probe-harness.py"
...
runner = load_module(RUNNER_PATH, "aghfal17_native_v9_runner_tests")
supervisor = load_module(SUPERVISOR_PATH, "aghfal17_native_v9_supervisor_tests")
bootstrap = load_module(BOOTSTRAP_PATH, "aghfal17_native_v9_bootstrap_tests")
harness = load_module(HARNESS_PATH, "aghfal17_native_v9_harness_tests")
...
class ProvenanceAndBoundaryTests(unittest.TestCase):
    ...
    def test_exact_stdlib_manifest_and_namespace_contract(self) -> None:
        raw = Path("/tmp/agent-aghfal17-native-v9-stdlib.sha256").read_bytes()
        ...
        minimal = Path("/tmp/agent-aghfal17-native-v9-minimal-tcb.sha256")
        self.assertEqual(
...
class RetainedReportTests(unittest.TestCase):
    def test_parent_retained_report_accepts_exact_named_inode(self) -> None:
        with tempfile.TemporaryDirectory(prefix="agh-v9-report-") as raw:
            directory = Path(raw)
    ...
    def test_same_uid_pathname_swap_is_rejected(self) -> None:
        with tempfile.TemporaryDirectory(prefix="agh-v9-swap-") as raw:
            directory = Path(raw)
            dirfd = os.open(directory, os.O_RDONLY | os.O_DIRECTORY)
            report_fd = os.open(
                "generic-validator-report.json",
...
class ProbeRejectionDiagnosticTests(unittest.TestCase):
    ...
    def _valid_payload(self) -> dict[str, object]:
        return {
            "schema": "planora.agh-fal17.native-v9-sealed-import-probe.v1",
            "status": "PASS",
...
class ResourceAndLifecycleTests(unittest.TestCase):
    ...
    def test_sigkill_harness_parent_death_chain_leaves_no_root_or_grandchild(self) -> None:
        with tempfile.TemporaryDirectory(dir="/tmp", prefix="agh-v9-pdeath-") as raw:
...
class PublicationTests(unittest.TestCase):
    def test_report_last_no_replace(self) -> None:
        with tempfile.TemporaryDirectory(prefix="agh-v9-publish-") as raw:
            directory = Path(raw)
...
class NoResultAcceptanceTests(unittest.TestCase):
    def test_no_result_publishes_nothing_and_xml_tamper_is_rejected(self) -> None:
        child = {
            "schema": "planora.agh-fal17.native-v9-runner.v1",
            "status": "NO_RESULT",
            "elapsed_seconds": 1.0,
        ...
        with tempfile.TemporaryDirectory(prefix="agh-v9-no-result-") as raw:
            directory = Path(raw)
