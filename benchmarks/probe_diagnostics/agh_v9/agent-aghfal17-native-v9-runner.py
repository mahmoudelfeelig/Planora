#!/usr/bin/env python3
"""Official-input-only AGH-FAL17 native solve runner v9."""
...
def _load_capture_manifest(
    ...
    if globals().get("__runner_loader_protocol__") != "planora.aghfal17.native-v9-runner-loader.v1":
...
def verify_runtime_bundle(
    ...
    if binding.get("protocol") != "planora.aghfal17.native-v9-sealed-runtime.v1":
    ...
    if (
        manifest.get("schema")
        != "planora.aghfal17.native-v9-sealed-runtime.v1"
...
def execute_completion() -> tuple[int, dict[str, Any]]:
    ...
    candidate = Path(os.environ["TMPDIR"]) / "native-v9-candidate.xml"
    ...
    report = {
        "schema": "planora.agh-fal17.native-v9-report.v1",
...
def self_test() -> dict[str, Any]:
    ...
    if forbidden & set(EXPECTED_HASHES):
        raise AssertionError("placement-bearing provenance label survived in v9")
...
def main() -> int:
    if not isinstance(globals().get("__captured_sha256__"), str):
        raise SystemExit("direct AGH-FAL17 native v9 runner execution rejected")
