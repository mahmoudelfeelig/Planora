#!/usr/bin/env python3
"""Authoritative whole-launch controller for the AGH-FAL17 v13 chain.
...
SCHEMA = "planora.agh-fal17.native-v13-outer-controller.v1"
FREEZE_SCHEMA = "planora.agh-fal17.native-v13-freeze.v1"
PR_SET_PDEATHSIG = 1
    ...
    *,
    ...
    while changed:
        ...
        for pid, member in admitted.items():
            ...
            if record is not None and record.identity == member.identity:
    ...
    return {
        ...
        "live_admitted_pids": sorted(
            pid
            for pid, member in admitted.items()
            if observed.get(pid) is not None
            and observed[pid].identity == member.identity
        ),
    }
...
def live_admitted(admitted: Mapping[int, AdmittedMember]) -> list[dict[str, Any]]:
    ...
    for pid, member in sorted(admitted.items()):
        record = proc_record(pid)
        if record is None or record.identity != member.identity:
            continue
        member.topology = record.topology
        rows.append(
            {
...
def _reap_known_children(
    ...
    for pid, member in sorted(admitted.items()):
        ...
        if (
            ...
            or record.topology.ppid != wrapper_pid
        ):
            continue
        member.topology = record.topology
        try:
...
def run_controlled(
    *,
    ...
    stdout_fd = os.memfd_create("aghfal17-v13-inner-stdout")
    stderr_fd = os.memfd_create("aghfal17-v13-inner-stderr")
    report_fd = os.memfd_create("aghfal17-v13-outer-report")
    ...
    try:
        ...
        environment = {
            ...
            "LC_ALL": "C.UTF-8",
            "TZ": "UTC",
            "AGHFAL_NATIVE_V13_OUTER_CONTROLLER": "1",
        }
