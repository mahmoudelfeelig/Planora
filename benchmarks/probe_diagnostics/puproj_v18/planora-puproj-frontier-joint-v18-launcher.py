"""Sealed v18 launcher that captures the supervisor before execution."""
...
ARTIFACT_ROOT = Path(
    "/mnt/d/Stuff/Projects/Sites/Planora/benchmarks/probe_diagnostics/puproj_v18"
)
SUPERVISOR = ARTIFACT_ROOT / "planora-puproj-frontier-joint-v18-supervisor.py"
EXPECTED_SUPERVISOR_SHA256 = (
    "6b43d34ad8056e863e0a164faac76ebc709c8acdaf09e34af9867792dd85c0f4"
)
REQUIRED_SEALS = (
...
def _read(descriptor: int, size: int) -> bytes:
    ...
    while offset < size:
        ...
        if not block:
            raise RuntimeError("PU-PROJ v18 supervisor source ended early")
        parts.append(block)
        offset += len(block)
    return b"".join(parts)
...
def _verify_bootstrap_handoff() -> tuple[dict[str, object], dict[str, object]]:
    if (
        ...
        raise RuntimeError("direct PU-PROJ v18 launcher execution rejected")
    ...
    if (
        ...
        raise RuntimeError("PU-PROJ v18 bootstrap handoff binding rejected")
    ...
    if (
        ...
        raise RuntimeError("PU-PROJ v18 sealed launcher replay rejected")
    ...
    if (
        ...
        raise RuntimeError("PU-PROJ v18 launcher source/watch binding rejected")
...
def main() -> int:
    ...
    try:
        ...
        if not stat.S_ISREG(before.st_mode) or before.st_nlink != 1:
            raise RuntimeError("PU-PROJ v18 supervisor source contract rejected")
        ...
        if _identity(after) != admitted or _identity(named) != admitted:
            raise RuntimeError("PU-PROJ v18 supervisor source identity drift")
        actual = sha256(source).hexdigest()
        if actual != EXPECTED_SUPERVISOR_SHA256:
            raise RuntimeError(f"PU-PROJ v18 supervisor hash drift: {actual}")
        sealed_fd = os.memfd_create(
            "planora-puproj-v18-supervisor", getattr(os, "MFD_ALLOW_SEALING", 0x0002)
        )
        ...
        while view:
            ...
            if written <= 0:
                raise RuntimeError(
                    "PU-PROJ v18 supervisor memfd stopped accepting bytes"
                )
            view = view[written:]
        ...
        if (
            ...
            raise RuntimeError("PU-PROJ v18 supervisor sealed replay rejected")
        filename = f"<sealed-puproj-frontier-v18-supervisor:{actual}>"
        namespace = {
            "__name__": "__main__",
            "__file__": filename,
            "__package__": None,
            ...
            "__external_loader_protocol__": "planora.puproj.frontier-v18-supervisor-loader.v1",
            "__external_loader_handoff_name__": "planora-puproj-frontier-joint-v18-launcher.py",
