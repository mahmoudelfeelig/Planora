...
def verify_system_python_provenance(
    *,
    ...
    expected_prefix_from_output = str(
        Path(str(output_binding.get("path"))) / ".pycache-v18"
    )
    if (
...
def load_capture_manifest(
    *, include_official: bool = True
    ...
    if (
        ...
        != "planora.puproj.frontier-v18-runner-loader.v1"
    ):
...
def verify_runtime_bundle(
    ...
    if binding.get("protocol") != "planora.puproj.frontier-v18-sealed-runtime.v1":
    ...
    if (
        manifest.get("schema") != "planora.puproj.frontier-v18-sealed-runtime.v1"
...
def admit_sealed_runtime_compile_warnings(
    ...
    return {
        "schema": "planora.puproj.frontier-joint-v18.compile-warnings.v1",
        "status": "ADMITTED",
        "count": 2,
        "category": "DeprecationWarning",
        "message": CP_MODEL_COMPILE_WARNING_MESSAGE,
...
def _serialize_solution(problem, placements, student_classes) -> bytes:
    ...
    root = ElementTree.Element(
        "solution", {"name": problem.name, "technique": "planora-puproj-v18-fresh"}
    )
...
class L(importlib.abc.Loader):
 ...
 def exec_module(self,m):
  m.__file__="sealed:puproj-v18-validator:"+self.n; m.__captured_sha256__=self.h
...
stdlib=json.loads(sys.argv[7]); forwarded=sys.argv[8:]; sys.argv=["sealed:puproj-v18-generic-validator",*forwarded]
scope={"__name__":"__main__","__file__":"sealed:puproj-v18-generic-validator","__package__":None}
try: exec(compile(validator,"sealed:puproj-v18-generic-validator","exec"),scope)
...
def _generic_validation(
    ...
    solution_fd = os.memfd_create(
        "puproj-v18-generic-solution", getattr(os, "MFD_ALLOW_SEALING", 2)
    )
...
def run_frontier() -> tuple[int, dict[str, Any]]:
    ...
    base = {
        "schema": "planora.pu-proj.frontier-joint-v18.fresh-report.v1",
    ...
    if not result.is_feasible or stopped["signal"] is not None:
        ...
        return 3, {
            "schema": "planora.pu-proj.frontier-joint-v18-runner.v1",
    ...
    solution_fd = os.memfd_create("puproj-v18-local-solution", getattr(os, "MFD_ALLOW_SEALING", 2))
    try:
        os.write(solution_fd, xml_bytes)
        os.fchmod(solution_fd, 0o400)
        fcntl.fcntl(solution_fd, fcntl.F_ADD_SEALS, REQUIRED_SEALS)
...
def run_sealed_import_probe() -> dict[str, Any]:
    ...
    return {
        "schema": "planora.puproj.frontier-joint-v18-sealed-import-probe-child.v1",
        "status": "PASS",
        "elapsed_seconds": time.monotonic() - started,
...
def main() -> int:
    if not isinstance(globals().get("__captured_sha256__"), str):
        raise SystemExit("direct PU-PROJ v18 runner execution rejected")
