...
EXPECTED_HASHES = {
    ...
    "itc2019_decomposed": "3f4b92f91867cd1205f1702f36923b3c19cb8ad8d39b43d34a3b15e07f502e05",
...
def verify_system_python_provenance(
    *,
    ...
    expected_prefix_from_output = str(
        Path(str(output_binding.get("path"))) / ".pycache-v19"
    )
    if (
...
def load_capture_manifest(
    *, include_official: bool = True
    ...
    if (
        ...
        != "planora.puproj.frontier-v19-runner-loader.v1"
    ):
...
class RuntimeBundleAdmission:
    ...
    compile_warnings: list[dict[str, Any]] | None = None
...
def verify_runtime_bundle(
    ...
    if binding.get("protocol") != "planora.puproj.frontier-v19-sealed-runtime.v1":
    ...
    if (
        manifest.get("schema") != "planora.puproj.frontier-v19-sealed-runtime.v1"
...
class _SealedSourceLoader:
    ...
    def get_filename(self, _fullname: str) -> str:
        return f"<sealed-runtime:{self.relative}:{self.entry['sha256']}>"
    ...
    def get_code(self, _fullname: str) -> Any:
        ...
        if self.bundle.compile_warnings is None:
            raise RuntimeError("sealed compile-warning sink absent")
        for item in captured:
            self.bundle.compile_warnings.append(
                {
                    "category": item.category.__name__,
                    "message": str(item.message),
                    "source_relative_path": self.relative,
                    "source_sha256": str(self.entry["sha256"]),
                }
            )
        return code
...
def admit_sealed_runtime_compile_warnings(
    bundle: RuntimeBundleAdmission,
) -> dict[str, Any]:
    """Admit only the two pinned CP-SAT source compile warnings."""

    source = bundle.entries_by_path.get(CP_MODEL_SOURCE_PATH)
    if (
        not isinstance(source, dict)
        or source.get("sha256") != CP_MODEL_SOURCE_SHA256
    ):
        raise RuntimeError("compile-warning CP-SAT source pin rejected")
    rows = bundle.compile_warnings
    if not isinstance(rows, list):
        raise RuntimeError("sealed compile-warning sink rejected")
    expected = {
        "category": "DeprecationWarning",
        "message": CP_MODEL_COMPILE_WARNING_MESSAGE,
        "source_relative_path": CP_MODEL_SOURCE_PATH,
        "source_sha256": CP_MODEL_SOURCE_SHA256,
    }
    if len(rows) != 2 or any(row != expected for row in rows):
        raise RuntimeError("sealed runtime compile-warning contract rejected")
    return {
...
def _normalize_loaded_runtime_rows(
    rows: list[dict[str, Any]],
) -> list[dict[str, Any]]:
    normalized_by_path: dict[str, dict[str, Any]] = {}
    for raw_row in rows:
        if (
            not isinstance(raw_row, dict)
            or set(raw_row) != {"path", "sha256", "size", "transport"}
            ...
            or any(
                ...
                for character in raw_row["sha256"]
            )
            or type(raw_row.get("size")) is not int
            or raw_row["size"] < 0
            or raw_row.get("transport") not in {
                "sealed_descriptor_loader",
                "sealed_native_descriptor",
            }
        ):
            raise RuntimeError("loaded runtime observation rejected")
        row = {
            "path": raw_row["path"],
            "sha256": raw_row["sha256"],
            "size": raw_row["size"],
            "transport": raw_row["transport"],
        }
        existing = normalized_by_path.get(row["path"])
        if existing is None:
            normalized_by_path[row["path"]] = row
        elif existing != row:
            raise RuntimeError("conflicting duplicate loaded runtime observation")
    return [normalized_by_path[path] for path in sorted(normalized_by_path)]


def _loaded_runtime_manifest(
    rows: list[dict[str, Any]],
) -> tuple[list[dict[str, Any]], str]:
    normalized = _normalize_loaded_runtime_rows(rows)
    digest = sha256(
...
def verify_loaded_runtime(
    ...
    loaded, combined = _loaded_runtime_manifest(loaded)
    return {
...
def prepare_run(
    ...
    *,
    ...
    compile_warnings = admit_sealed_runtime_compile_warnings(runtime_bundle)
...
def _serialize_solution(problem, placements, student_classes) -> bytes:
    ...
    root = ElementTree.Element(
        "solution", {"name": problem.name, "technique": "planora-puproj-v19-fresh"}
    )
...
class L(importlib.abc.Loader):
 ...
 def exec_module(self,m):
  m.__file__="sealed:puproj-v19-validator:"+self.n; m.__captured_sha256__=self.h
...
stdlib=json.loads(sys.argv[7]); forwarded=sys.argv[8:]; sys.argv=["sealed:puproj-v19-generic-validator",*forwarded]
scope={"__name__":"__main__","__file__":"sealed:puproj-v19-generic-validator","__package__":None}
try: exec(compile(validator,"sealed:puproj-v19-generic-validator","exec"),scope)
...
def _generic_validation(
    ...
    solution_fd = os.memfd_create(
        "puproj-v19-generic-solution", getattr(os, "MFD_ALLOW_SEALING", 2)
    )
...
def run_frontier() -> tuple[int, dict[str, Any]]:
    ...
    base = {
        "schema": "planora.pu-proj.frontier-joint-v19.fresh-report.v1",
    ...
    if not result.is_feasible or stopped["signal"] is not None:
        ...
        return 3, {
            "schema": "planora.pu-proj.frontier-joint-v19-runner.v1",
    ...
    solution_fd = os.memfd_create("puproj-v19-local-solution", getattr(os, "MFD_ALLOW_SEALING", 2))
    try:
        os.write(solution_fd, xml_bytes)
        os.fchmod(solution_fd, 0o400)
        fcntl.fcntl(solution_fd, fcntl.F_ADD_SEALS, REQUIRED_SEALS)
...
def main() -> int:
    if not isinstance(globals().get("__captured_sha256__"), str):
        raise SystemExit("direct PU-PROJ v19 runner execution rejected")
