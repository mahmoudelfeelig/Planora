#!/usr/bin/env python3
"""Synthetic/adversarial gates for MUNI-FSPSX frontier v19.
...
BOOTSTRAP = CHAIN_ROOT / "planora-muni-fspsx-frontier-v19-bootstrap.py"
INLINE_TRUST = CHAIN_ROOT / "planora-muni-fspsx-frontier-v19-inline-trust-root.txt"
...
runner = load(RUNNER, "planora_muni_v19_runner_tests")
supervisor = load(SUPERVISOR, "planora_muni_v19_supervisor_tests")
bootstrap = load(BOOTSTRAP, "planora_muni_v19_bootstrap_tests")
...
def synthetic_runtime_source_tree(sources: dict[str, bytes]):
    with tempfile.TemporaryDirectory(dir="/tmp", prefix="muni-v19-cache-release-") as raw:
        root = Path(raw)
        ...
        for relative, value in sources.items():
            ...
            source.chmod(0o444)
        record_fd = memfd("synthetic-runtime-record", runtime_record(sources))
        root_fd = os.open(bundle_root, os.O_RDONLY | os.O_DIRECTORY)
        try:
            yield site_packages, root_fd, {"runtime_test_record": {"fd": record_fd}}
        finally:
            os.close(root_fd)
            os.close(record_fd)
...
class StaticContractTests(unittest.TestCase):
    ...
    def test_v19_preserves_all_v18_resource_floors_and_limits(self) -> None:
    ...
    def test_current_quality_closure_and_certificate_paths_are_unambiguous(self) -> None:
        manifest = json.loads(
            (CHAIN_ROOT / "planora-muni-fspsx-frontier-v19-freeze-manifest.json").read_bytes()
        )
        ...
        self.assertEqual(
            ...
            str(CHAIN_ROOT / "planora-muni-fspsx-frontier-v19-certificate.json"),
        )
...
class RuntimeClosureTests(unittest.TestCase):
    ...
    def test_real_sealed_runtime_imports_ortools_without_live_site_packages(self) -> None:
        ...
        completed = subprocess.run(
            [
                str(PYTHON), "-I", "-S", "-B", "-X",
                "pycache_prefix=/tmp/muni-v19-probe-pyc",
    ...
    def test_cache_release_occurs_after_stable_read_verification_and_sealing(self) -> None:
        ...
        with synthetic_runtime_source_tree(sources) as (
            ...
            original_read = supervisor._pread_stable
            original_seal = supervisor._seal_bytes

            def tracked_read(descriptor: int, *, maximum_bytes: int) -> bytes:
                value = original_read(descriptor, maximum_bytes=maximum_bytes)
                try:
                    opened = Path(os.readlink(f"/proc/self/fd/{descriptor}")).resolve()
                except OSError:
                    opened = Path("/")
                if opened == source_path:
                    events.append("stable_read_complete")
                return value

            def tracked_seal(name: str, raw: bytes):
                result = original_seal(name, raw)
                if name.startswith("muni-v19-runtime-") and not name.endswith(
                    "manifest"
                ):
                    events.append("sealed_copy_complete")
                return result
            ...
            with (
                ...
                mock.patch.object(supervisor, "_pread_stable", side_effect=tracked_read),
                mock.patch.object(supervisor, "_seal_bytes", side_effect=tracked_seal),
            ...
            try:
                self.assertEqual(
                    events,
                    [
                        "stable_read_complete",
                        "sealed_copy_complete",
    ...
    def test_cache_release_is_not_attempted_before_stable_read_completes(self) -> None:
        ...
        with synthetic_runtime_source_tree(sources) as (
            ...
            def fail_source_read(descriptor: int, *, maximum_bytes: int) -> bytes:
                try:
                    opened = Path(os.readlink(f"/proc/self/fd/{descriptor}")).resolve()
                except OSError:
                    opened = Path("/")
                if opened == (site_packages / "example/package.py").resolve():
                    raise RuntimeError("synthetic incomplete stable read")
            ...
            with (
                ...
                mock.patch.object(supervisor, "LAUNCH_MEMAVAILABLE_FLOOR_KIB", 0),
                mock.patch.object(supervisor, "_pread_stable", side_effect=fail_source_read),
    ...
    def test_real_record_bundle_builds_and_replays_without_official_sources(self) -> None:
        ...
        runtime_fds: list[int] = []
        with tempfile.TemporaryDirectory(dir="/tmp", prefix="muni-v19-runtime-test-") as raw:
            root = Path(raw)
...
class SealedImportProbeTests(unittest.TestCase):
    ...
    def test_real_chain_reaches_probe_admission_without_opening_inputs(self) -> None:
        ...
        manifest_path = CHAIN_ROOT / "planora-muni-fspsx-frontier-v19-freeze-manifest.json"
...
class ArtifactSchemaTests(unittest.TestCase):
    ...
    def partial(self) -> dict[str, object]:
        return {
            "schema": "planora.muni-fspsx.frontier-v19.fresh-partial.v1",
    ...
    def test_controlled_unknown_schema_is_exact(self) -> None:
        payload = {
            "schema": "planora.muni-fspsx.frontier-v19.controlled-unknown.v1",
...
def runtime_import_probe() -> None:
    ...
    capture_fds: list[int] = []
    runtime_fds: list[int] = []
    with tempfile.TemporaryDirectory(dir="/tmp", prefix="muni-v19-import-probe-") as raw:
