#!/usr/bin/env python3
"""Synthetic/adversarial gates for MUNI-FSPSX frontier v16.
...
BOOTSTRAP = CHAIN_ROOT / "planora-muni-fspsx-frontier-v16-bootstrap.py"
INLINE_TRUST = CHAIN_ROOT / "planora-muni-fspsx-frontier-v16-inline-trust-root.txt"
...
runner = load(RUNNER, "planora_muni_v16_runner_tests")
supervisor = load(SUPERVISOR, "planora_muni_v16_supervisor_tests")
bootstrap = load(BOOTSTRAP, "planora_muni_v16_bootstrap_tests")
...
class StaticContractTests(unittest.TestCase):
    ...
    def test_current_quality_closure_and_certificate_paths_are_unambiguous(self) -> None:
        manifest = json.loads(
            (CHAIN_ROOT / "planora-muni-fspsx-frontier-v16-freeze-manifest.json").read_bytes()
        )
        ...
        self.assertEqual(
            ...
            str(CHAIN_ROOT / "planora-muni-fspsx-frontier-v16-certificate.json"),
        )
        fairness = manifest["fairness_provenance"]
        self.assertNotIn("certificate_path", fairness)
        self.assertEqual(
            fairness["derivation_audit_path"],
            str(CHAIN_ROOT / "planora-muni-fspsx-v35-derivation-audit-v1.json"),
        )
...
class RuntimeClosureTests(unittest.TestCase):
    def test_real_sealed_runtime_imports_ortools_without_live_site_packages(self) -> None:
        ...
        completed = subprocess.run(
            [
                str(PYTHON), "-I", "-S", "-B", "-X",
                "pycache_prefix=/tmp/muni-v16-probe-pyc",
    ...
    def test_real_record_bundle_builds_and_replays_without_official_sources(self) -> None:
        ...
        with tempfile.TemporaryDirectory(dir="/tmp", prefix="muni-v16-runtime-test-") as raw:
            root = Path(raw)
...
class SealedImportProbeTests(unittest.TestCase):
    ...
    def test_real_chain_reaches_probe_admission_without_opening_inputs(self) -> None:
        ...
        manifest_path = CHAIN_ROOT / "planora-muni-fspsx-frontier-v16-freeze-manifest.json"
...
class ArtifactSchemaTests(unittest.TestCase):
    ...
    def partial(self) -> dict[str, object]:
        return {
            "schema": "planora.muni-fspsx.frontier-v16.fresh-partial.v1",
    ...
    def test_controlled_unknown_schema_is_exact(self) -> None:
        payload = {
            "schema": "planora.muni-fspsx.frontier-v16.controlled-unknown.v1",
...
def runtime_import_probe() -> None:
    ...
    capture_fds: list[int] = []
    runtime_fds: list[int] = []
    with tempfile.TemporaryDirectory(dir="/tmp", prefix="muni-v16-import-probe-") as raw:
