#!/usr/bin/env python3
"""Attribution-safe immutable supervisor for the MUNI-FSPSX v16 diagnostic."""
...
CHAIN_ROOT = ROOT / "benchmarks/probe_diagnostics/muni_v16"
PYTHON = Path("/usr/bin/python3.12")
SITE_PACKAGES = ROOT / ".venv/lib/python3.12/site-packages"
SUPERVISOR = CHAIN_ROOT / "planora-muni-fspsx-frontier-v16-supervisor.py"
LAUNCHER = CHAIN_ROOT / "planora-muni-fspsx-frontier-v16-launcher.sh"
RUNNER = CHAIN_ROOT / "planora-muni-fspsx-frontier-v16-runner.py"
FREEZE_MANIFEST = CHAIN_ROOT / "planora-muni-fspsx-frontier-v16-freeze-manifest.json"
STDLIB_MANIFEST = CHAIN_ROOT / "planora-muni-fspsx-frontier-v16-stdlib.sha256"
...
RUNNER_LOADER_PROTOCOL = "planora.muni-fspsx.frontier-v16-runner-loader.v1"
RUNTIME_BUNDLE_PROTOCOL = "planora.muni-fspsx.frontier-v16-sealed-runtime.v1"
...
def _stream_capture(path: Path, expected: str, label: str) -> tuple[int, dict[str, Any]]:
    ...
    try:
        ...
        target_fd = os.memfd_create(f"muni-v16-{label}", getattr(os, "MFD_ALLOW_SEALING", 0x0002))
        digest = sha256()
        offset = 0
    ...
    *, runtime_root_fd: int, captures: Mapping[str, Mapping[str, Any]]
    ...
    try:
        for index, (relative, (expected, expected_size, record_label)) in enumerate(
            ...
            if index % 64 == 0:
                sample = host_sample()
                if sample["mem_available_kib"] < LAUNCH_MEMAVAILABLE_FLOOR_KIB:
                    raise RuntimeError("runtime bundle launch MemAvailable floor")
            source_fd = os.open(
                relative,
                os.O_RDONLY | getattr(os, "O_NOFOLLOW", 0),
        ...
        manifest = {
            "schema": "planora.muni-fspsx.frontier-v16-sealed-runtime.v1",
        ...
        manifest_fd, manifest_identity, manifest_seals = _seal_bytes(
            "muni-v16-runtime-manifest", manifest_raw
        )
        binding = {
            "protocol": "planora.muni-fspsx.frontier-v16-sealed-runtime.v1",
            "root_fd": root_fd,
            "root_identity": list(root_identity),
        ...
        if post_capture_sample["mem_available_kib"] < LAUNCH_MEMAVAILABLE_FLOOR_KIB:
            raise RuntimeError("runtime bundle post-capture MemAvailable floor")
        summary = {
...
def verify_runtime_bundle_end(binding: Mapping[str, Any]) -> dict[str, Any]:
    ...
    for row in manifest["entries"]:
        ...
        if (
            ...
            or sha256(payload).hexdigest() != row.get("sha256")
            or len(payload) != row.get("size")
            or tuple(
                row.get(key)
                for key in (
...
def load_freeze_manifest(expected_sha256: str, *, external: bool) -> tuple[dict[str, object], bytes]:
    ...
    if not isinstance(payload, dict) or payload.get("schema") != "planora.muni-fspsx.frontier-v16.freeze.v1":
        raise RuntimeError("unsupported v16 freeze manifest")
...
def validate_manifest_contract(manifest: Mapping[str, object]) -> None:
    ...
    if (
        ...
        != "planora.muni-fspsx.frontier-v16.inline-trust-argv.v1"
...
def manifest_files(manifest: Mapping[str, object]) -> dict[str, dict[str, object]]:
    ...
    if Path(str(rows["bootstrap"]["path"])) != Path(
        str(CHAIN_ROOT / "planora-muni-fspsx-frontier-v16-bootstrap.py")
    ):
    ...
    if (
        ...
        != CHAIN_ROOT / "planora-muni-fspsx-frontier-v16-minimal-tcb.sha256"
...
def resource_decision(
    *,
    ...
    if supervisor_memory_kib + group_memory_kib > WHOLE_LAUNCH_MEMORY_CAP_KIB:
...
def read_process_memory_status_once(pid: int) -> dict[str, int]:
    ...
    try:
        ...
        raise RuntimeError(f"process status missing for identity-pinned PID {pid}") from exc
    values: dict[str, int] = {}
    state: str | None = None
    for line in lines:
        fields = line.split()
        ...
        if len(fields) >= 2 and fields[0] in {"VmRSS:", "VmSwap:"}:
            try:
                ...
                raise RuntimeError(f"process status memory value rejected for PID {pid}") from exc
    if state == "Z" and not values:
        return {"VmRSS": 0, "VmSwap": 0}
    if state is None or set(values) != {"VmRSS", "VmSwap"}:
        raise RuntimeError(f"process status memory fields missing for PID {pid}")
    return values
...
def identity_pinned_probe_telemetry_sample(
    ...
    *,
    ...
    decision = sealed_import_probe_iteration_decision(
        ...
        whole_memory_kib=int(memory["whole_launch_set_union_memory_kib"]),
    ...
    return {
        ...
        "whole_launch_set_union_memory_kib": int(
            memory["whole_launch_set_union_memory_kib"]
        ),
...
def stop_process_group(
    ...
    def observe_wait(timeout: float, stage: str) -> tuple[int | None, bool]:
        """Observe/reap without allowing a wait fault to skip later cleanup."""
    ...
    # This is the final admission opportunity and occurs before wait(), which
    # can reap a zombie leader and remove the original-generation anchor.
    refresh_process_group_generation(generation)
    # Reap a known exited Popen leader before interpreting the numeric PID in
    # the group enumeration.  An unreaped zombie is still listed in /proc and
    # is not PID reuse.  timeout=0 never waits for a running leader.
    _initial_returncode, known_leader_reaped = observe_wait(
        0, "initial_wait"
    )
    snapshot_before = admitted_generation_snapshot(generation)
    ...
    term_signal = signal_process_group_snapshot(
        pgid, snapshot_before, signal.SIGTERM
    )
    deadline = time.monotonic() + TERMINATION_GRACE_SECONDS
    group_observed_empty = not snapshot_before
    while time.monotonic() < deadline:
        refresh_process_group_generation(generation)
        if process.returncode is None:
            observe_wait(0, "grace_wait")
        if not admitted_generation_snapshot(generation):
            group_observed_empty = True
            break
        group_observed_empty = False
        time.sleep(0.05)
    survivor_snapshot = (
        () if group_observed_empty else admitted_generation_snapshot(generation)
    )
    survivors = tuple(pid for pid, _identity in survivor_snapshot)
    kill_signal = signal_process_group_snapshot(
        pgid, survivor_snapshot, signal.SIGKILL
    )
    ...
    final_admitted_survivors = tuple(
        pid for pid, _identity in admitted_generation_snapshot(generation)
    )
    current_group = process_group_snapshot(pgid)
    unregistered_current_members = tuple(
        pid
        for pid, identity in current_group
        if generation.members.get(pid) != identity
    )
    ...
    if final_admitted_survivors or unregistered_current_members:
        cleanup_errors.append(
            ...
            + ",".join(
                str(pid)
                for pid in (*final_admitted_survivors, *unregistered_current_members)
            )
        )
    return {
        ...
        "pid_reuse_guard_passed": not unregistered_current_members,
        "leader_exited_before_cleanup": leader_exited_before_cleanup,
        "original_pgid_asserted_empty": not (
            final_admitted_survivors or unregistered_current_members
        ),
        "group_observed_empty_before_any_later_signal": group_observed_empty,
...
def validate_partial_payload(payload: object) -> bool:
    ...
    return bool(
        ...
        == "planora.muni-fspsx.frontier-v16.fresh-partial.v1"
...
def validate_controlled_stdout(payload: object) -> bool:
    ...
    return bool(
        ...
        and payload.get("schema")
        == "planora.muni-fspsx.frontier-v16.controlled-unknown.v1"
...
def validate_complete_report(payload: object) -> bool:
    ...
    return bool(
        ...
        and payload.get("schema")
        == "planora.muni-fspsx.frontier-v16.fresh-complete.v1"
...
def classify_artifacts(
    *,
    ...
    complete_stdout = bool(
        ...
        == "planora.muni-fspsx.frontier-v16.runner-result.v1"
...
def run_launch(
    ...
    try:
        ...
        run_directory = Path(f"/tmp/planora-muni-fspsx-frontier-v16-{run_id}")
        ...
        runtime_directory = Path(
            f"/tmp/planora-muni-fspsx-frontier-v16-runtime-{run_id}"
        )
        ...
        pre_child_supervisor_memory = supervisor_memory_kib()
        peak_supervisor_memory = max(
            peak_supervisor_memory, pre_child_supervisor_memory
        )
        peak_whole_launch_memory = max(
            peak_whole_launch_memory, pre_child_supervisor_memory
        )
        if pre_child_supervisor_memory > WHOLE_LAUNCH_MEMORY_CAP_KIB:
        ...
        while True:
            ...
            if process.poll() is not None:
                break
            sample = host_sample()
            group_memory, members = process_group_memory_kib(pgid)
            own_memory = supervisor_memory_kib()
            whole_memory = own_memory + group_memory
    ...
    def monitor_payload() -> dict[str, object]:
        return {
        "schema": "planora.muni-fspsx.frontier-v16.supervisor.v1",
...
def build_probe_rejection_diagnostics(
    *,
    ...
    return {
        "schema": "planora.muni-fspsx.frontier-v16.rejection-diagnostics.v1",
        "child_started": child_started,
        "child_exit": child_exit,
...
def run_sealed_import_probe(
    ...
    if time.monotonic() > absolute_deadline:
        ...
        print(json.dumps({
            "schema": "planora.muni-fspsx.frontier-v16.sealed-import-probe-gate.v1",
    ...
    try:
        ...
        run_id = configured_run_id()
        run_directory = Path(
            f"/tmp/planora-muni-fspsx-frontier-v16-probe-{run_id}"
        )
        os.mkdir(run_directory, 0o700)
        run_directory_fd = os.open(
            run_directory,
        ...
        for label, (path, expected) in capture_sources.items():
            ...
            pass_fds.append(descriptor)
        runtime_directory = Path(
            f"/tmp/planora-muni-fspsx-frontier-v16-probe-runtime-{run_id}"
        )
        os.mkdir(runtime_directory, 0o700)
        runtime_initial_fd = os.open(
            runtime_directory,
        ...
        peak_whole = max(peak_whole, pre_child_memory)
        if pre_child_memory > WHOLE_LAUNCH_MEMORY_CAP_KIB:
        ...
        expected_child = {
            "schema": "planora.muni-fspsx.frontier-v16.sealed-import-probe-child.v1",
            "status": "PASS",
            "input_mode": "NONE",
            "official_input_opened": False,
            "progress_opened": False,
    ...
    if accepted and run_directory is not None and run_directory_fd is not None and run_identity is not None:
        monitor = {
            "schema": "planora.muni-fspsx.frontier-v16.sealed-import-probe.v1",
            "status": "PASS",
...
def dry_run(expected_manifest_sha256: str | None) -> dict[str, object]:
    ...
    try:
        ...
        errors.append(f"{type(exc).__name__}: {exc}")
    run_directory = Path(f"/tmp/planora-muni-fspsx-frontier-v16-{configured_run_id()}")
    if os.path.lexists(run_directory):
        errors.append(f"planned run directory exists: {run_directory}")
    return {
        "schema": "planora.muni-fspsx.frontier-v16.supervisor-dry.v1",
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    if not args.launch:
        ...
        print(json.dumps({
            "schema": "planora.muni-fspsx.frontier-v16.supervisor-gate.v1",
            "status": "NOT_LAUNCHED",
            "children_started": False,
            "artifacts_written": False,
