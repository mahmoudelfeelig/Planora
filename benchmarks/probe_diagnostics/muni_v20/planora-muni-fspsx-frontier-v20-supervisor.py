#!/usr/bin/env python3
"""Attribution-safe immutable supervisor for the MUNI-FSPSX v20 diagnostic."""
...
CHAIN_ROOT = ROOT / "benchmarks/probe_diagnostics/muni_v20"
PYTHON = Path("/usr/bin/python3.12")
SITE_PACKAGES = ROOT / ".venv/lib/python3.12/site-packages"
SUPERVISOR = CHAIN_ROOT / "planora-muni-fspsx-frontier-v20-supervisor.py"
LAUNCHER = CHAIN_ROOT / "planora-muni-fspsx-frontier-v20-launcher.sh"
RUNNER = CHAIN_ROOT / "planora-muni-fspsx-frontier-v20-runner.py"
FREEZE_MANIFEST = CHAIN_ROOT / "planora-muni-fspsx-frontier-v20-freeze-manifest.json"
STDLIB_MANIFEST = CHAIN_ROOT / "planora-muni-fspsx-frontier-v20-stdlib.sha256"
...
RUNNER_LOADER_PROTOCOL = "planora.muni-fspsx.frontier-v20-runner-loader.v1"
RUNTIME_BUNDLE_PROTOCOL = "planora.muni-fspsx.frontier-v20-sealed-runtime.v1"
...
RUNTIME_RESOURCE_PROTOCOL = (
    "planora.muni-fspsx.frontier-v20-runtime-resource-checkpoints.v1"
)
...
sys.argv=["sealed:muni-v20-runner",*child_argv]
scope={"__name__":"__main__","__package__":None,"__file__":"sealed:muni-v20-runner","__planora_runner_evidence__":{"fd":runner_fd,"sha256":digest},"__planora_stdlib_evidence__":{"fd":stdlib_fd,"sha256":stdlib_digest,"pre_runner_admitted":True},"__planora_source_evidence__":evidence,"__captured_sha256__":digest,"__runner_loader_protocol__":"planora.muni-fspsx.frontier-v20-runner-loader.v1"}
exec(compile(source,"sealed:muni-v20-runner","exec"),scope)
'''
...
def _stream_capture(path: Path, expected: str, label: str) -> tuple[int, dict[str, Any]]:
    ...
    try:
        ...
        target_fd = os.memfd_create(f"muni-v20-{label}", getattr(os, "MFD_ALLOW_SEALING", 0x0002))
        digest = sha256()
        offset = 0
...
def build_runtime_bundle(
    *, runtime_root_fd: int, captures: Mapping[str, Mapping[str, Any]]
    ...
    try:
        ...
        manifest = {
            "schema": "planora.muni-fspsx.frontier-v20-sealed-runtime.v1",
        ...
        manifest_fd, manifest_identity, manifest_seals = _seal_bytes(
            "muni-v20-runtime-manifest", manifest_raw
        )
        ...
        binding = {
            "protocol": "planora.muni-fspsx.frontier-v20-sealed-runtime.v1",
            "root_fd": root_fd,
            "root_identity": list(root_identity),
...
def load_freeze_manifest(expected_sha256: str, *, external: bool) -> tuple[dict[str, object], bytes]:
    ...
    if not isinstance(payload, dict) or payload.get("schema") != "planora.muni-fspsx.frontier-v20.freeze.v1":
        raise RuntimeError("unsupported v20 freeze manifest")
...
def validate_manifest_contract(manifest: Mapping[str, object]) -> None:
    ...
    if (
        ...
        != "planora.muni-fspsx.frontier-v20.inline-trust-argv.v1"
...
def manifest_files(manifest: Mapping[str, object]) -> dict[str, dict[str, object]]:
    ...
    if Path(str(rows["bootstrap"]["path"])) != Path(
        str(CHAIN_ROOT / "planora-muni-fspsx-frontier-v20-bootstrap.py")
    ):
    ...
    if (
        ...
        != CHAIN_ROOT / "planora-muni-fspsx-frontier-v20-minimal-tcb.sha256"
...
def validate_partial_payload(payload: object) -> bool:
    ...
    return bool(
        ...
        == "planora.muni-fspsx.frontier-v20.fresh-partial.v1"
...
def validate_controlled_stdout(payload: object) -> bool:
    ...
    return bool(
        ...
        and payload.get("schema")
        == "planora.muni-fspsx.frontier-v20.controlled-unknown.v1"
...
def validate_complete_report(payload: object) -> bool:
    ...
    return bool(
        ...
        and payload.get("schema")
        == "planora.muni-fspsx.frontier-v20.fresh-complete.v1"
...
def classify_artifacts(
    *,
    ...
    complete_stdout = bool(
        ...
        == "planora.muni-fspsx.frontier-v20.runner-result.v1"
...
def run_launch(
    ...
    try:
        ...
        run_directory = Path(f"/tmp/planora-muni-fspsx-frontier-v20-{run_id}")
        ...
        runtime_directory = Path(
            f"/tmp/planora-muni-fspsx-frontier-v20-runtime-{run_id}"
        )
    ...
    def monitor_payload() -> dict[str, object]:
        return {
        "schema": "planora.muni-fspsx.frontier-v20.supervisor.v1",
...
def build_probe_rejection_diagnostics(
    *,
    ...
    return {
        "schema": "planora.muni-fspsx.frontier-v20.rejection-diagnostics.v1",
        "child_started": child_started,
        "child_exit": child_exit,
...
def run_sealed_import_probe(
    ...
    if time.monotonic() > absolute_deadline:
        ...
        print(json.dumps({
            "schema": "planora.muni-fspsx.frontier-v20.sealed-import-probe-gate.v1",
    ...
    try:
        ...
        run_id = configured_run_id()
        run_directory = Path(
            f"/tmp/planora-muni-fspsx-frontier-v20-probe-{run_id}"
        )
        os.mkdir(run_directory, 0o700)
        run_directory_fd = os.open(
            run_directory,
        ...
        for label, (path, expected) in capture_sources.items():
            ...
            pass_fds.append(descriptor)
        runtime_directory = Path(
            f"/tmp/planora-muni-fspsx-frontier-v20-probe-runtime-{run_id}"
        )
        os.mkdir(runtime_directory, 0o700)
        runtime_initial_fd = os.open(
            runtime_directory,
        ...
        expected_child = {
            "schema": "planora.muni-fspsx.frontier-v20.sealed-import-probe-child.v1",
            "status": "PASS",
            "input_mode": "NONE",
            "official_input_opened": False,
            "progress_opened": False,
            ...
            "solve_called": False,
            "compile_warnings": {
                "schema": "planora.muni-fspsx.frontier-v20.compile-warnings.v1",
                "status": "ADMITTED",
                "count": 2,
    ...
    if accepted and run_directory is not None and run_directory_fd is not None and run_identity is not None:
        monitor = {
            "schema": "planora.muni-fspsx.frontier-v20.sealed-import-probe.v1",
            "status": "PASS",
...
def dry_run(expected_manifest_sha256: str | None) -> dict[str, object]:
    ...
    try:
        ...
        errors.append(f"{type(exc).__name__}: {exc}")
    run_directory = Path(f"/tmp/planora-muni-fspsx-frontier-v20-{configured_run_id()}")
    if os.path.lexists(run_directory):
        errors.append(f"planned run directory exists: {run_directory}")
    return {
        "schema": "planora.muni-fspsx.frontier-v20.supervisor-dry.v1",
...
def pre_child_manifest_contract_check(
    ...
    return {
        "schema": (
            "planora.muni-fspsx.frontier-v20."
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    if not args.launch:
        ...
        print(json.dumps({
            "schema": "planora.muni-fspsx.frontier-v20.supervisor-gate.v1",
            "status": "NOT_LAUNCHED",
            "children_started": False,
            "artifacts_written": False,
