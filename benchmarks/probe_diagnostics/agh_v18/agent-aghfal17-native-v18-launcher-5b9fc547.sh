...
readonly expected_launcher_sha256=${AGHFAL_NATIVE_V18_LAUNCHER_SHA256:-}
readonly admitted_bootstrap_sha256=${AGHFAL_NATIVE_V18_BOOTSTRAP_SHA256:-}
readonly admitted_bash_sha256=${AGHFAL_NATIVE_V18_BASH_SHA256:-}
readonly admitted_bootstrap_fd=${AGHFAL_NATIVE_V18_BOOTSTRAP_FD:-}
readonly admitted_launcher_fd=${AGHFAL_NATIVE_V18_LAUNCHER_FD:-}
readonly admitted_bash_fd=${AGHFAL_NATIVE_V18_BASH_FD:-}
if [[ "${AGHFAL_NATIVE_V18_SEALED_LAUNCHER:-}" != "1" \
   ...
   || "${BASH_SOURCE[0]}" != /proc/self/fd/* ]]; then
    echo "AGH-FAL17 v18 launcher requires the sealed bootstrap" >&2
    exit 70
fi
...
if [[ "$actual_launcher_sha256" != "$expected_launcher_sha256" ]]; then
    echo "AGH-FAL17 v18 sealed launcher digest drift" >&2
    exit 70
fi
...
if [[ "$actual_python_sha256" != "$expected_python_sha256" ]]; then
    echo "AGH-FAL17 v18 launcher Python hash drift" >&2
    exit 70
fi

exec /usr/bin/env -i PATH=/usr/bin:/bin LANG=C.UTF-8 LC_ALL=C.UTF-8 TZ=UTC \
    AGHFAL_NATIVE_V18_LAUNCHER_SHA256="$actual_launcher_sha256" \
    AGHFAL_NATIVE_V18_BOOTSTRAP_SHA256="$admitted_bootstrap_sha256" \
    AGHFAL_NATIVE_V18_BOOTSTRAP_FD="$admitted_bootstrap_fd" \
    AGHFAL_NATIVE_V18_BASH_SHA256="$admitted_bash_sha256" \
    AGHFAL_NATIVE_V18_BASH_FD="$admitted_bash_fd" \
    AGHFAL_NATIVE_V18_LAUNCHER_FD="$admitted_launcher_fd" \
    AGHFAL_NATIVE_V18_SEALED_LAUNCHER=1 \
    "/proc/$$/fd/9" -I -S -B -c '
import fcntl
import hashlib
import os
...
stdlib_path = "/tmp/agent-aghfal17-native-v18-stdlib.sha256"
...
if not root_read_only:
    raise RuntimeError("AGH-FAL17 v18 stdlib filesystem is not read-only")
...
if hashlib.sha256(stdlib_raw).hexdigest() != stdlib_expected:
    raise RuntimeError("AGH-FAL17 v18 stdlib manifest drift")
stdlib = {}
for line in stdlib_raw.decode("utf-8").splitlines():
    ...
    if file_path in stdlib or not file_path.startswith("/usr/lib/python3.12/"):
        raise RuntimeError("AGH-FAL17 v18 stdlib manifest row rejected")
...
for file_path, file_hash in stdlib.items():
    ...
    while True:
        ...
        if (
            ...
            raise RuntimeError("AGH-FAL17 v18 stdlib ownership/permissions rejected")
        if current == "/":
            break
    ...
    if (
        ...
        raise RuntimeError("AGH-FAL17 v18 stdlib file drift")
for module in tuple(sys.modules.values()):
    ...
    if os.path.realpath(file_path) != file_path or file_path not in stdlib:
        raise RuntimeError("AGH-FAL17 v18 pre-supervisor module outside stdlib closure")

path = sys.argv[1]
expected = sys.argv[2]
launcher_path = sys.argv[3]
if set(os.environ) != {"PATH", "LANG", "LC_ALL", "TZ", "AGHFAL_NATIVE_V18_LAUNCHER_SHA256", "AGHFAL_NATIVE_V18_BOOTSTRAP_SHA256", "AGHFAL_NATIVE_V18_BASH_SHA256", "AGHFAL_NATIVE_V18_BOOTSTRAP_FD", "AGHFAL_NATIVE_V18_LAUNCHER_FD", "AGHFAL_NATIVE_V18_BASH_FD", "AGHFAL_NATIVE_V18_SEALED_LAUNCHER"}:
    raise RuntimeError("AGH-FAL17 v18 launcher environment was not sanitized")
launcher_fd = int(launcher_path.removeprefix("/proc/self/fd/"))
if launcher_fd != int(os.environ["AGHFAL_NATIVE_V18_LAUNCHER_FD"]):
    raise RuntimeError("AGH-FAL17 v18 sealed launcher descriptor handoff drift")
...
while offset < launcher_row.st_size:
    ...
    if not block: raise RuntimeError("AGH-FAL17 v18 sealed launcher ended early")
    chunks.append(block); offset += len(block)
launcher_raw = b"".join(chunks)
launcher_sha256 = hashlib.sha256(launcher_raw).hexdigest()
if launcher_sha256 != os.environ["AGHFAL_NATIVE_V18_LAUNCHER_SHA256"] or launcher_seals & required_seals != required_seals:
    raise RuntimeError("AGH-FAL17 v18 sealed launcher binding drift")
sealed_rows = {}
for label in ("BOOTSTRAP", "LAUNCHER"):
    descriptor = int(os.environ[f"AGHFAL_NATIVE_V18_{label}_FD"])
    ...
    while offset < row.st_size:
        ...
        if not block: raise RuntimeError(f"AGH-FAL17 v18 sealed {label.lower()} ended early")
        chunks.append(block); offset += len(block)
    digest = hashlib.sha256(b"".join(chunks)).hexdigest()
    try:
        seals = int(fcntl.fcntl(descriptor, fcntl.F_GET_SEALS))
    except OSError as exc:
        raise RuntimeError(f"AGH-FAL17 v18 sealed {label.lower()} descriptor replay unavailable") from exc
    if not stat.S_ISREG(row.st_mode) or digest != os.environ[f"AGHFAL_NATIVE_V18_{label}_SHA256"] or seals & required_seals != required_seals:
        raise RuntimeError(f"AGH-FAL17 v18 sealed {label.lower()} replay drift")
...
if len(expected) != 64 or any(value not in "0123456789abcdef" for value in expected):
    raise RuntimeError("frozen AGH-FAL17 v18 supervisor SHA-256 is invalid")
flags = os.O_RDONLY
...
try:
    ...
    if not stat.S_ISREG(before.st_mode):
        raise RuntimeError("AGH-FAL17 v18 supervisor path is not regular")
    identity = lambda row: (
    ...
    while offset < before.st_size:
        ...
        if not block:
            raise RuntimeError("AGH-FAL17 v18 supervisor source ended early")
        chunks.append(block)
        offset += len(block)
    after = os.fstat(descriptor)
finally:
...
if (
    ...
    raise RuntimeError("AGH-FAL17 v18 supervisor regular-file contract changed")
source = b"".join(chunks)
actual = hashlib.sha256(source).hexdigest()
if actual != expected:
    raise RuntimeError(f"AGH-FAL17 v18 supervisor hash drift: {actual} != {expected}")
filename = f"<captured-aghfal17-native-v18-supervisor:{actual}>"
...
namespace = {
    ...
    "__external_loader_protocol__": "planora.aghfal17.native-v18-supervisor-loader.v1",
    "__external_loader_handoff_name__": "agent-aghfal17-native-v18-launcher.sh",
    "__external_launcher_attestation__": {
        "protocol": "planora.aghfal17.native-v18-sealed-launcher-bootstrap.v1",
        ...
        "bash_sha256": os.environ["AGHFAL_NATIVE_V18_BASH_SHA256"],
        "bash_fd": int(os.environ["AGHFAL_NATIVE_V18_BASH_FD"]),
...
' /tmp/agent-aghfal17-native-v18-supervisor-5b9fc547.py df6604025812858768b9e334729419afb99b096b0b6274d5bd9eace6a36d7481 "${BASH_SOURCE[0]}" "$@"
