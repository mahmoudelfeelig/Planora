#!/usr/bin/env python3
"""Official-input-only AGH-FAL17 native solve runner v11."""
    ...
    for path, expected in rows.items():
        ...
        while current not in verified_ancestors:
            ...
            if (
                ...
                raise RuntimeError(
                    "stdlib owner or writable ancestor rejected: " + current
                )
    ...
    for module in tuple(sys.modules.values()):
        ...
        if resolved.startswith("/usr/lib/python3.12/"):
            if resolved != path or rows.get(resolved) is None:
                raise RuntimeError(
                    "loaded stdlib module outside exact manifest: " + path
                )
...
def _load_capture_manifest(
    ...
    if (
        ...
        != "planora.aghfal17.native-v18-runner-loader.v1"
    ):
...
def load_probe_capture_manifest() -> tuple[dict[str, bytes], dict[str, dict[str, Any]]]:
...
def verify_runtime_bundle(
    ...
    if binding.get("protocol") != "planora.aghfal17.native-v18-sealed-runtime.v1":
    ...
    if (
        manifest.get("schema") != "planora.aghfal17.native-v18-sealed-runtime.v1"
...
class _SealedRuntimeFinder:
    ...
    def find_spec(self, fullname: str, _path: Any = None, _target: Any = None) -> Any:
        ...
        if package_relative in self.bundle.entries_by_path:
            ...
            return _importlib_util.spec_from_loader(fullname, loader, is_package=True)
        if module_relative in self.bundle.entries_by_path:
            ...
            return _importlib_util.spec_from_loader(fullname, loader, is_package=False)
        for suffix in _importlib_machinery.EXTENSION_SUFFIXES:
            ...
            loader = _importlib_machinery.ExtensionFileLoader(fullname, exact_path)
    ...
    *,
    ...
    for line in Path("/proc/self/maps").read_text(encoding="utf-8").splitlines():
        ...
        if key in sealed_identities:
            ...
            if "site-packages" in decoded_parts or "dist-packages" in decoded_parts:
                raise RuntimeError(
    ...
    for module in tuple(sys.modules.values()):
        ...
        if "site-packages" in raw_parts or "dist-packages" in raw_parts:
            raise RuntimeError(f"live third-party Python module rejected: {raw_path}")
...
def verify_loaded_runtime(
    ...
    for module in tuple(sys.modules.values()):
        ...
        try:
            ...
            if path.is_absolute() and not _admitted_system_python_module_path(raw_path):
                unexpected.add(raw_path)
            continue
...
def install_sealed_planora_modules(payloads: Mapping[str, bytes]) -> dict[str, Any]:
    ...
    if already_loaded:
        raise RuntimeError(
            "Planora modules loaded before sealed admission: "
            + ",".join(already_loaded)
        )
...
def verify_loaded_planora_modules() -> dict[str, str]:
    ...
    for name, label in PLANORA_MODULE_LABELS.items():
        ...
        if getattr(module, "__captured_sha256__", None) != expected or not str(
            getattr(module, "__file__", "")
        ).startswith(f"<sealed-planora:{name}:"):
...
def _runtime_replay(
    *,
    ...
    final_maps = mapped_runtime_snapshot(runtime_bundle, capture_evidence, phase=phase)
...
def execute_completion() -> tuple[int, dict[str, Any]]:
    ...
    if not result.is_feasible:
        ...
        return 2, {
            "schema": "planora.agh-fal17.native-v18-runner.v1",
            "status": "NO_RESULT",
            ...
            "stdlib_final": verify_stdlib_manifest(payloads, phase="final_no_result"),
    ...
    if (
        ...
        raise RuntimeError(
            "native result failed exact local cardinality/semantic gates"
        )
    candidate = Path(os.environ["TMPDIR"]) / "native-v18-candidate.xml"
    ...
    stdlib_final = verify_stdlib_manifest(payloads, phase="final_before_publication")
    report = {
        "schema": "planora.agh-fal17.native-v18-report.v1",
...
def self_test() -> dict[str, Any]:
    ...
    if forbidden & set(EXPECTED_HASHES):
        raise AssertionError("placement-bearing provenance label survived in v11")
...
def sealed_import_probe() -> dict[str, Any]:
    ...
    final_maps = mapped_runtime_snapshot(runtime_bundle, captures, phase="probe_final")
...
def main() -> int:
    if not isinstance(globals().get("__captured_sha256__"), str):
        raise SystemExit("direct AGH-FAL17 native v11 runner execution rejected")
