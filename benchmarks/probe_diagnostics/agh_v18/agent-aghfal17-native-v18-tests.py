#!/usr/bin/env python3
"""Focused adversarial tests for the AGH-FAL17 v18 outer control plane.
...
OUTER_PATH = ARTIFACT_ROOT / "agent-aghfal17-native-v18-outer-controller.py"
FREEZE_PATH = ARTIFACT_ROOT / "agent-aghfal17-native-v18-review-freeze.json"
INVOCATIONS_PATH = ARTIFACT_ROOT / "agent-aghfal17-native-v18-invocations.json"
BOOTSTRAP_PATH = ARTIFACT_ROOT / "agent-aghfal17-native-v18-bootstrap.py"
SUPERVISOR_PATH = ARTIFACT_ROOT / "agent-aghfal17-native-v18-supervisor.py"
RUNNER_PATH = ARTIFACT_ROOT / "agent-aghfal17-native-v18-runner.py"
LAUNCHER_PATH = ARTIFACT_ROOT / "agent-aghfal17-native-v18-launcher.sh"
...
outer = load(OUTER_PATH, "aghfal17_native_v18_outer_tested")
...
class V18FreezeReadinessTests(unittest.TestCase):
    @classmethod
    def setUpClass(cls) -> None:
        cls.freeze = json.loads(FREEZE_PATH.read_text(encoding="utf-8"))
        cls.invocations = json.loads(INVOCATIONS_PATH.read_text(encoding="utf-8"))

    def test_v18_is_review_ready_but_execution_unauthorized(self) -> None:
        self.assertEqual(
            self.freeze["status"],
