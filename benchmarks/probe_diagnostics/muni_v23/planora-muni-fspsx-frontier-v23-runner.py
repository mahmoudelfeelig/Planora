#!/usr/bin/env python3
"""Frozen source-only MUNI-FSPSX v23 frontier diagnostic runner.
...
def load_capture_manifest(
    ...
    if globals().get("__runner_loader_protocol__") != "planora.muni-fspsx.frontier-v23-runner-loader.v1":
...
def _verify_cache_release_telemetry(
    ...
    *,
    ...
    expected = _expected_cache_release_advisories(rows)
    if telemetry != expected:
        raise RuntimeError("sealed runtime cache-release telemetry mismatch")
    actual_sha256 = sha256(
        json.dumps(expected, sort_keys=True, separators=(",", ":")).encode("utf-8")
    ).hexdigest()
    if expected_sha256 != actual_sha256:
        raise RuntimeError("sealed runtime cache-release telemetry hash mismatch")
    return {
...
def verify_runtime_bundle(
    ...
    if binding.get("protocol") != "planora.muni-fspsx.frontier-v23-sealed-runtime.v1":
    ...
    if (
        manifest.get("schema")
        != "planora.muni-fspsx.frontier-v23-sealed-runtime.v1"
    ...
    if (
        ...
        raise RuntimeError("sealed runtime cache-release telemetry pin mismatch")
    if (
        ...
        raise RuntimeError("sealed runtime cache-release binding rejected")
    if manifest.get("resource_checkpoint_protocol") != RUNTIME_RESOURCE_PROTOCOL:
        raise RuntimeError("sealed runtime resource checkpoint protocol rejected")
    ...
    if (
        ...
        raise RuntimeError("sealed runtime resource checkpoint binding rejected")
    ...
    if (
        ...
        raise RuntimeError("sealed runtime final resource binding rejected")
    entries_by_path: dict[str, dict[str, Any]] = {}
    entries_by_identity: dict[tuple[int, int], dict[str, Any]] = {}
    ...
    for row in rows:
        ...
        identity = _stable_identity(before)
        seals = int(fcntl.fcntl(descriptor, fcntl.F_GET_SEALS))
        observed_sha256, observed_size, observed_identity = _pread_digest_stable(
            descriptor,
        ...
        if (
            ...
            or observed_sha256 != expected_digest
...
def admit_sealed_runtime_compile_warnings(
    ...
    manifest_source_sha256 = source.get("sha256")
    if (
        not isinstance(manifest_source_sha256, str)
        or len(manifest_source_sha256) != 64
        or manifest_source_sha256 != CP_MODEL_SOURCE_SHA256
    ):
        raise RuntimeError("compile-warning CP-SAT source pin rejected")
...
def run_sealed_import_probe(args: argparse.Namespace) -> int:
    ...
    compile_warnings = admit_sealed_runtime_compile_warnings(bundle)
...
def run(args: argparse.Namespace) -> int:
    ...
    partial_payload = {
        "schema": "planora.muni-fspsx.frontier-v23.fresh-partial.v1",
    ...
    try:
        ...
        report = {
            "schema": "planora.muni-fspsx.frontier-v23.fresh-complete.v1",
        ...
        print(
            json.dumps(
                {
                    "schema": "planora.muni-fspsx.frontier-v23.runner-result.v1",
                    "status": "COMPLETE_VALID",
                    "admissible_as_solution": True,
                    "competitor_schedule_or_result_used": False,
                    "competitor_placement_or_hint_used": False,
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    if not args.launch:
        print(json.dumps({
            "schema": "planora.muni-fspsx.frontier-v23.runner-gate.v1",
            "status": "NOT_LAUNCHED",
