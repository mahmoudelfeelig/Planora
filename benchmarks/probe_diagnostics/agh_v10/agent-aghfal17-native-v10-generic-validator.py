#!/usr/bin/env python3
"""Distinct isolated semantic validator for AGH-FAL17 native v10."""
...
def _load_runner() -> types.ModuleType:
    ...
    module = types.ModuleType("aghfal17_native_v10_generic_runtime")
    module.__file__ = f"<sealed-native-v10-runner:{actual}>"
...
def validate(solution_fd: int) -> dict[str, object]:
    ...
    return {
        "schema": "planora.agh-fal17.native-v10-isolated-generic-validator.v1",
...
def main() -> int:
    if globals().get("__generic_loader_protocol__") != (
        "planora.aghfal17.native-v10-generic-loader.v1"
    ):
