...
class PersistenceStore:
    ...
    def resolve_principal(self, principal: Principal) -> Principal:
        with self._connect() as conn:
            ...
            bindings = conn.execute(
                ...
                """,
                (principal.tenant_id, principal.user_id, principal.tenant_id, principal.user_id),
            ).fetchall()
            roles.extend(
                str(row["role"])
                for row in bindings
        ...
        *,
        ...
        with self._connect() as conn:
            ...
            conn.execute(
                ...
                (tenant_id, group_id, user_id, now),
            )
            conn.execute(
    ...
    def access_snapshot(self, principal: Principal) -> Dict[str, Any]:
        ...
        return {"users": users, "groups": groups, "memberships": memberships, "role_bindings": bindings, "invite_codes": invites}
