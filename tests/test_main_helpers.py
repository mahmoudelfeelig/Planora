...
def test_normalize_instance_preserves_staff_availability_and_drops_sunday(capsys):
    groups = {
    ...
    assert inst.days == ["MON", "TUE"]
    assert staff[1].available_days == {"MON"}

    out = capsys.readouterr().out
    assert "Removed Sunday" in out
...
def test_check_staff_weekly_capacity_respects_daily_limit(capsys):
    groups = {1: Group(id=1, name="G1", program_id=1, size=20, course_ids=[1])}
    courses = {
        1: Course(
            id=1,
    ...
    out = capsys.readouterr().out
    assert "[WARN] Staff 'Prof-1' (PROF) week 1: need 3 slots > capacity 2" in out
