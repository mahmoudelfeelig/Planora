...
$runId = '5f2d84640f40404a82dd180d7043d9c5'
$root = "/tmp/planora-muni-v31-canonical-tests-$runId"
$snapshotRepo = '/snapshot/repo'
$runnerRelative = 'scripts/run_muni_v31_canonical_tests.ps1'
$runnerPath = Join-Path $repo 'scripts\run_muni_v31_canonical_tests.ps1'
$authorizationRelative = 'output/diagnostic-receipts/muni-fspsx-v31-canonical-tests-authorization-20260828T113225Z.receipt.json'
$authorizationPath = Join-Path $repo 'output\diagnostic-receipts\muni-fspsx-v31-canonical-tests-authorization-20260828T113225Z.receipt.json'
...
$prefix = Join-Path $repo "output\diagnostic-receipts\muni-fspsx-v31-canonical-readonly-tests-$runId"
$prefixWsl = "$repoWsl/output/diagnostic-receipts/muni-fspsx-v31-canonical-readonly-tests-$runId"
...
function Get-ExpectedAuthorizationJson([long]$RunnerSize,[string]$RunnerHash){
    ...
    $o=[ordered]@{
        schema='planora.itc2019.canonical-test-authorization.v11';created_at_utc='2026-08-28T11:32:25Z';instance='muni-fspsx-fal17';candidate='muni_v31';test_id=$runId
        ...
        successor_admission=[ordered]@{builder=[ordered]@{path='scripts/build_muni_v31_successor.py';size=69258;sha256='c08338a076d08336f704e4fe364cdd15ba78904bbc9b4cdc2a96d262e493079e'};tests=[ordered]@{path='tests/test_run_muni_v31_successor.py';size=27636;sha256='be3eb15ba49c3c9b0a5ed0c6d579ab0e83775dc054ff3e9e65ffdef48be5c9fb'}}
...
def inventory():
 ...
 return {'schema':'planora.muni-v31.snapshot-inventory.v1','root':root,'directory_count':len(dirs),'file_count':len(files),'total_bytes':total,'directories':sorted(dirs,key=lambda x:x['path']),'files':sorted(files,key=lambda x:x['path'])}
if contract.get('schema')!='planora.muni-v31.snapshot-closure.v4': raise RuntimeError('snapshot contract schema rejected')
 ...
 for name in sorted(leaves):
  ...
  with open(path,'rb',buffering=0) as f:
   while True:
    ...
    h.update(b)
  files.append({'path':r,'type':'regular file','mode':'0400','device':s.st_dev,'inode':s.st_ino,'nlink':1,'size':s.st_size,'sha256':h.hexdigest()});total+=s.st_size
inv={'schema':'planora.muni-v31.snapshot-inventory.v1','root':root,'directory_count':len(dirs),'file_count':len(files),'total_bytes':total,'directories':sorted(dirs,key=lambda x:x['path']),'files':sorted(files,key=lambda x:x['path'])}
...
try:
 ...
 if shutdown.get('schema')!='planora.muni-v31.watcher-shutdown-control.v2' or shutdown.get('run_id')!=run_id or shutdown.get('inventory_sha256')!=expected_sha or shutdown.get('acceptance_commitment_sha256')!=control['acceptance_commitment_sha256'] or not isinstance(shutdown.get('cleanup_evidence_sha256'),str) or len(shutdown['cleanup_evidence_sha256'])!=64 or not isinstance(shutdown.get('protected_replay_sha256'),str) or len(shutdown['protected_replay_sha256'])!=64 or not isinstance(shutdown.get('pass_receipt_sha256'),str) or len(shutdown['pass_receipt_sha256'])!=64: raise RuntimeError('watcher shutdown control rejected')
...
$cleanupSource = @'
import hashlib,json,os,stat,sys
c=json.loads(__import__('base64').b64decode(sys.argv[1]));root=c['root'];expected_path=c['expected'];prefix='/tmp/planora-muni-v31-canonical-tests-'
...
if(-not('PlanoraV31NativeFileIdentity'-as[type])){
Add-Type -TypeDefinition @"
using System;
using System.Runtime.InteropServices;
using Microsoft.Win32.SafeHandles;
[StructLayout(LayoutKind.Sequential)]
public struct PlanoraV31FileInformation {
...
public static class PlanoraV31NativeFileIdentity {
 [DllImport("kernel32.dll", SetLastError=true)]
 public static extern bool GetFileInformationByHandle(SafeFileHandle handle, out PlanoraV31FileInformation information);
}
"@
}
function Get-HeldFileIdentity([IO.FileStream]$Stream,[string]$Label){
    $information=New-Object PlanoraV31FileInformation;if(-not[PlanoraV31NativeFileIdentity]::GetFileInformationByHandle($Stream.SafeFileHandle,[ref]$information)){$code=[Runtime.InteropServices.Marshal]::GetLastWin32Error();throw [IO.IOException]::new("$Label GetFileInformationByHandle failed: $code")}
...
if($LogBridgeSelfTest){
    $auth=Get-AuthorizationState;$name='muni-v31-log-bridge-'+[Guid]::NewGuid().ToString('N');$dir=Join-Path $repo ('tmp\'+$name);$dirWsl=$repoWsl+'/tmp/'+$name
    ...
    try{
        ...
        if($stableReads-lt3){throw 'Fixed bridge did not overlap at least three stable host reads'};[Console]::Out.WriteLine(([ordered]@{schema='planora.muni-v31.log-bridge-self-test.v1';status='PASS';run_id=$runId;runner_sha256=$auth.runner_sha256;legacy_lifetime_open_sharing_failure='REPRODUCED';short_lived_append_stable_reads=$stableReads;rows=$rows.Count;samples=12;duplicates=0;canonical_suite_executed=$false;shared_lock_used=$false}|ConvertTo-Json -Compress));return
...
try{
    ...
    $predecessorEvidence=Get-ValidatedCombinedPredecessorEvidence $true;$staleArchivePin=$predecessorEvidence.contract.v28.archive;$predecessorPins=@(Get-CombinedPredecessorPinArray $predecessorEvidence);if($predecessorPins.Count-ne41){throw 'Combined predecessor pin cardinality rejected'}
