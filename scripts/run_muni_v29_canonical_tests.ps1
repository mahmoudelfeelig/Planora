...
$runId = 'ca79220da7db46b6996fe1f05785dde7'
$root = "/tmp/planora-muni-v29-canonical-tests-$runId"
$snapshotRepo = '/snapshot/repo'
$runnerRelative = 'scripts/run_muni_v29_canonical_tests.ps1'
...
$v28ClaimPath = Join-Path $repo 'output\diagnostic-receipts\muni-fspsx-v28-canonical-readonly-tests-e7cf1df162074402994a9d0ad763c824.claim.json'
$v28RejectionPath = Join-Path $repo 'output\diagnostic-receipts\muni-fspsx-v28-canonical-readonly-tests-e7cf1df162074402994a9d0ad763c824.rejection.json'
$v28StaticPath = Join-Path $repo 'output\diagnostic-receipts\muni-fspsx-v28-canonical-readonly-tests-e7cf1df162074402994a9d0ad763c824.static-adversarial.json'
...
function Get-ExpectedAuthorizationJson([long]$RunnerSize,[string]$RunnerHash){
    ...
    $o=[ordered]@{
        ...
        successor_admission=[ordered]@{builder=[ordered]@{path='scripts/build_muni_v29_successor.py';size=65112;sha256='ec1cf8b79dc57d322f7f79a359bf378471a311122e40c92b7da3c7407d241208'};tests=[ordered]@{path='tests/test_run_muni_v29_successor.py';size=14889;sha256='4bbb2726b7d0adb4751423e153421ff3b8707482e5dbc92790e7919440446705'}}
        pinned_v28_files=[ordered]@{
            ...
            runner=[ordered]@{path='scripts/run_muni_v28_canonical_tests.ps1';size=161962;sha256='fbf0a2f4449806cec331c71efc79417553f2d1cd6b060f5d481a32dbfc896d60'}
            ...
            claim=[ordered]@{path='output/diagnostic-receipts/muni-fspsx-v28-canonical-readonly-tests-e7cf1df162074402994a9d0ad763c824.claim.json';size=541;sha256='f0ba301e63ba7e96938dabd3473106114d3b29ac2ad2090222609e8cbc1432e4'}
            rejection=[ordered]@{path='output/diagnostic-receipts/muni-fspsx-v28-canonical-readonly-tests-e7cf1df162074402994a9d0ad763c824.rejection.json';size=1081;sha256='2ecfb0ba960173f5662dd423e7fd1c72ace10f6c281bdd50e3dcef130179ee41'}
            static_evidence=[ordered]@{path='output/diagnostic-receipts/muni-fspsx-v28-canonical-readonly-tests-e7cf1df162074402994a9d0ad763c824.static-adversarial.json';size=2019;sha256='93724cbeb1a76d199424309fee2a57df139bf70234ec93d8fc4fbbd6d5be7adf'}
        ...
        predecessor_failure=[ordered]@{run_id='e7cf1df162074402994a9d0ad763c824';status='REJECTED_AUTHORIZATION_CONSUMED';claim_sha256='f0ba301e63ba7e96938dabd3473106114d3b29ac2ad2090222609e8cbc1432e4';rejection_sha256='2ecfb0ba960173f5662dd423e7fd1c72ace10f6c281bdd50e3dcef130179ee41';static_evidence_sha256='93724cbeb1a76d199424309fee2a57df139bf70234ec93d8fc4fbbd6d5be7adf';pass_receipt_absent=$true;shutdown_seal_absent=$true;root_cause='held_lock_path_reopened_for_hash_caused_self_sharing_violation_before_canonical_tests'}
...
def inventory():
 ...
 return {'schema':'planora.muni-v29.snapshot-inventory.v1','root':root,'directory_count':len(dirs),'file_count':len(files),'total_bytes':total,'directories':sorted(dirs,key=lambda x:x['path']),'files':sorted(files,key=lambda x:x['path'])}
if contract.get('schema')!='planora.muni-v29.snapshot-closure.v4': raise RuntimeError('snapshot contract schema rejected')
 ...
 for name in sorted(leaves):
  ...
  with open(path,'rb',buffering=0) as f:
   while True:
    ...
    h.update(b)
  files.append({'path':r,'type':'regular file','mode':'0400','device':s.st_dev,'inode':s.st_ino,'nlink':1,'size':s.st_size,'sha256':h.hexdigest()});total+=s.st_size
inv={'schema':'planora.muni-v29.snapshot-inventory.v1','root':root,'directory_count':len(dirs),'file_count':len(files),'total_bytes':total,'directories':sorted(dirs,key=lambda x:x['path']),'files':sorted(files,key=lambda x:x['path'])}
...
try:
 ...
 if shutdown.get('schema')!='planora.muni-v29.watcher-shutdown-control.v2' or shutdown.get('run_id')!=run_id or shutdown.get('inventory_sha256')!=expected_sha or shutdown.get('acceptance_commitment_sha256')!=control['acceptance_commitment_sha256'] or not isinstance(shutdown.get('cleanup_evidence_sha256'),str) or len(shutdown['cleanup_evidence_sha256'])!=64 or not isinstance(shutdown.get('protected_replay_sha256'),str) or len(shutdown['protected_replay_sha256'])!=64 or not isinstance(shutdown.get('pass_receipt_sha256'),str) or len(shutdown['pass_receipt_sha256'])!=64: raise RuntimeError('watcher shutdown control rejected')
...
function Assert-V28StaleLockRecord([object]$Record){
    if($Record.schema-cne'planora.shared-heavy-wsl-lock.v1'-or$Record.run_id-cne'e7cf1df162074402994a9d0ad763c824'-or$Record.authorization_sha256-cne'1e2ac9d1edfe7ee5191c631834eaa36b7b59d6886e94887aedc189e3c098026d'-or$Record.runner_sha256-cne'fbf0a2f4449806cec331c71efc79417553f2d1cd6b060f5d481a32dbfc896d60'-or$Record.mechanism-cne'FileMode.CreateNew_held_open'-or[int]$Record.owner_pid-ne1140){throw 'Pinned v28 stale-lock schema/binding rejected'}
    return $true
}
function New-ExpectedV28PredecessorEvidence{
    return [ordered]@{
        schema='planora.muni-v29.complete-v28-predecessor-evidence.v1';run_id='e7cf1df162074402994a9d0ad763c824';status='EXPECTED_UNVALIDATED'
        ...
        pass_evidence=[ordered]@{receipt_path='output/diagnostic-receipts/muni-fspsx-v28-canonical-readonly-tests-e7cf1df162074402994a9d0ad763c824.receipt.json';shutdown_seal_path='output/diagnostic-receipts/muni-fspsx-v28-canonical-readonly-tests-e7cf1df162074402994a9d0ad763c824.pass-publication-shutdown-seal.json';receipt_absent_required=$true;shutdown_seal_absent_required=$true}
...
function Get-ValidatedV28PredecessorEvidence{
    ...
    if($claim.run_id-cne'e7cf1df162074402994a9d0ad763c824'-or$claim.status-cne$e.failure_semantics.claim_status-or-not[bool]$claim.failure_consumes_authorization-or$rejection.run_id-cne'e7cf1df162074402994a9d0ad763c824'-or$rejection.status-cne$e.failure_semantics.status-or$rejection.claim_sha256-cne$e.failure_semantics.claim_sha256-or[bool]$rejection.pass_receipt_present-or-not[bool]$rejection.pass_shutdown_seal_absent-or$static.run_id-cne'e7cf1df162074402994a9d0ad763c824'-or$static.runner_sha256-cne'fbf0a2f4449806cec331c71efc79417553f2d1cd6b060f5d481a32dbfc896d60'-or$static.authorization_sha256-cne'1e2ac9d1edfe7ee5191c631834eaa36b7b59d6886e94887aedc189e3c098026d'){throw 'Pinned v28 consumed-failure semantics rejected'}
...
function Invoke-StaleLockAdversarialModel{
    $good=[pscustomobject]@{schema='planora.shared-heavy-wsl-lock.v1';run_id='e7cf1df162074402994a9d0ad763c824';authorization_sha256='1e2ac9d1edfe7ee5191c631834eaa36b7b59d6886e94887aedc189e3c098026d';runner_sha256='fbf0a2f4449806cec331c71efc79417553f2d1cd6b060f5d481a32dbfc896d60';mechanism='FileMode.CreateNew_held_open';owner_pid=1140}
...
function Reconcile-PinnedV28StaleLock([object]$PredecessorEvidence){
    ...
    $claim=[IO.File]::ReadAllText($v28ClaimPath,$utf8)|ConvertFrom-Json;$rejection=[IO.File]::ReadAllText($v28RejectionPath,$utf8)|ConvertFrom-Json;$static=[IO.File]::ReadAllText($v28StaticPath,$utf8)|ConvertFrom-Json
    if($claim.run_id-cne'e7cf1df162074402994a9d0ad763c824'-or$claim.status-cne'CLAIMED_FAIL_CLOSED_UNLESS_VALID_PASS_PUBLICATION_SHUTDOWN_SEAL_EXISTS'-or-not[bool]$claim.failure_consumes_authorization-or$rejection.run_id-cne'e7cf1df162074402994a9d0ad763c824'-or$rejection.status-cne'REJECTED_AUTHORIZATION_CONSUMED'-or$rejection.claim_sha256-cne'f0ba301e63ba7e96938dabd3473106114d3b29ac2ad2090222609e8cbc1432e4'-or[bool]$rejection.pass_receipt_present-or-not[bool]$rejection.pass_shutdown_seal_absent-or$static.run_id-cne'e7cf1df162074402994a9d0ad763c824'-or$static.runner_sha256-cne'fbf0a2f4449806cec331c71efc79417553f2d1cd6b060f5d481a32dbfc896d60'-or$static.authorization_sha256-cne'1e2ac9d1edfe7ee5191c631834eaa36b7b59d6886e94887aedc189e3c098026d'){throw 'Pinned v28 consumed-failure semantics rejected'}
...
try{
    ...
    Assert-LocalPin $successorBuilderPath 65112 'ec1cf8b79dc57d322f7f79a359bf378471a311122e40c92b7da3c7407d241208'
    Assert-LocalPin $admissionTestsPath 14889 '4bbb2726b7d0adb4751423e153421ff3b8707482e5dbc92790e7919440446705'
