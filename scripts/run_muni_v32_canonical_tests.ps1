param([switch]$StaticSelfTest,[switch]$EmitExpectedAuthorization,[switch]$LogBridgeSelfTest,[switch]$ReadinessPredicateSelfTest,[switch]$RetainedV30SnapshotSelfTest,[switch]$RetainedPredecessorSnapshotsSelfTest,[switch]$CanonicalMonitorContractSelfTest,[switch]$RejectionPromotionSelfTest)
...
$runId = '4dc45edcd74446909290afadd5d3ecf0'
$root = "/tmp/planora-muni-v32-canonical-tests-$runId"
$snapshotRepo = '/snapshot/repo'
$runnerRelative = 'scripts/run_muni_v32_canonical_tests.ps1'
$runnerPath = Join-Path $repo 'scripts\run_muni_v32_canonical_tests.ps1'
$authorizationRelative = 'output/diagnostic-receipts/muni-fspsx-v32-canonical-tests-authorization-20260828T130114Z.receipt.json'
$authorizationPath = Join-Path $repo 'output\diagnostic-receipts\muni-fspsx-v32-canonical-tests-authorization-20260828T130114Z.receipt.json'
...
$prefix = Join-Path $repo "output\diagnostic-receipts\muni-fspsx-v32-canonical-readonly-tests-$runId"
$prefixWsl = "$repoWsl/output/diagnostic-receipts/muni-fspsx-v32-canonical-readonly-tests-$runId"
$sharedLockPath = Join-Path $repo 'output\diagnostic-receipts\planora-shared-heavy-wsl.lock'
$successorBuilderPath = Join-Path $repo 'scripts\build_muni_v32_successor.py'
$admissionTestsPath = Join-Path $repo 'tests\test_run_muni_v32_successor.py'
...
function Invoke-ReadinessPredicateRegression{
    $dir=Join-Path ([IO.Path]::GetTempPath()) ('planora-v32-readiness-'+[Guid]::NewGuid().ToString('N'));$missing=Join-Path $dir 'missing.log';$empty=Join-Path $dir 'empty.log'
...
function Get-ExpectedAuthorizationJson([long]$RunnerSize,[string]$RunnerHash){
    ...
    $o=[ordered]@{
        schema='planora.itc2019.canonical-test-authorization.v12';created_at_utc='2026-08-28T13:01:14Z';instance='muni-fspsx-fal17';candidate='muni_v32';test_id=$runId
        decision='GO_FOR_EXACTLY_ONE_CANONICAL_IMMUTABLE_SNAPSHOT_SUITE_AFTER_AUTHENTICATED_V31_ARGV_PIPELINE_FAILURE';retained_probe_authorized=$false;official_input_authorized=$false;official_launch_authorized=$false;solver_authorized=$false;publication_authorized=$false;automatic_retry_authorized=$false
        runner=[ordered]@{path=$runnerRelative;size=$RunnerSize;sha256=$RunnerHash}
        successor_admission=[ordered]@{builder=[ordered]@{path='scripts/build_muni_v32_successor.py';size=67244;sha256='0f40634fd7521318b375225a60bc3d1a935bc0a952915f5a3a0857c7d827ba9a'};tests=[ordered]@{path='tests/test_run_muni_v32_successor.py';size=28634;sha256='2875be0c8c64d48ff2fe5557b3a7e1e7f43ebd14232a226ac078cb0e0a96e693'}}
        ...
        retained_v30_snapshot_contract=[ordered]@{root=$predecessor.v30.snapshot.root;inventory=$predecessor.v30.snapshot.inventory;isolated_switch='RetainedV30SnapshotSelfTest';initial_and_terminal_read_only_replay_required=$true;post_cleanup_replay_while_v32_lock_held=$true;terminal_replay_immediately_before_final_archive_guard=$true;descriptor_open_nofollow_identity_replay=$true;v32_cleanup_must_not_target_v30=$true}
        retained_predecessor_snapshots_contract=[ordered]@{v30_root=$predecessor.v30.snapshot.root;v31_root=$v31Failure.snapshot.root;v30_inventory=$predecessor.v30.snapshot.inventory;v31_inventory=$v31Failure.snapshot.inventory;isolated_switch='RetainedPredecessorSnapshotsSelfTest';read_only_identity_replay=$true;initial_post_cleanup_rejection_and_final_replay_required=$true;v32_cleanup_must_not_target_v30_or_v31=$true}
...
def inventory():
 ...
 return {'schema':'planora.muni-v32.snapshot-inventory.v1','root':root,'directory_count':len(dirs),'file_count':len(files),'total_bytes':total,'directories':sorted(dirs,key=lambda x:x['path']),'files':sorted(files,key=lambda x:x['path'])}
if contract.get('schema')!='planora.muni-v32.snapshot-closure.v4': raise RuntimeError('snapshot contract schema rejected')
...
print(json.dumps({'schema':'planora.muni-v32.retained-snapshot-replay.v1','status':'EXACT_RETAINED_SNAPSHOT_REPLAY','phase':phase,'root':root,'files':len(files),'directories':len(dirs),'bytes':total,'inventory_sha256':hashlib.sha256(current_raw).hexdigest(),'all_nlink_one':all(x['nlink']==1 for x in files)},sort_keys=True,separators=(',',':')))
'@
 ...
 for name in sorted(leaves):
  ...
  with open(path,'rb',buffering=0) as f:
   while True:
    ...
    h.update(b)
  files.append({'path':r,'type':'regular file','mode':'0400','device':s.st_dev,'inode':s.st_ino,'nlink':1,'size':s.st_size,'sha256':h.hexdigest()});total+=s.st_size
inv={'schema':'planora.muni-v32.snapshot-inventory.v1','root':root,'directory_count':len(dirs),'file_count':len(files),'total_bytes':total,'directories':sorted(dirs,key=lambda x:x['path']),'files':sorted(files,key=lambda x:x['path'])}
...
try:
 ...
 if shutdown.get('schema')!='planora.muni-v32.watcher-shutdown-control.v2' or shutdown.get('run_id')!=run_id or shutdown.get('inventory_sha256')!=expected_sha or shutdown.get('acceptance_commitment_sha256')!=control['acceptance_commitment_sha256'] or not isinstance(shutdown.get('cleanup_evidence_sha256'),str) or len(shutdown['cleanup_evidence_sha256'])!=64 or not isinstance(shutdown.get('protected_replay_sha256'),str) or len(shutdown['protected_replay_sha256'])!=64 or not isinstance(shutdown.get('pass_receipt_sha256'),str) or len(shutdown['pass_receipt_sha256'])!=64: raise RuntimeError('watcher shutdown control rejected')
...
$cleanupSource = @'
import hashlib,json,os,stat,sys
c=json.loads(__import__('base64').b64decode(sys.argv[1]));root=c['root'];expected_path=c['expected'];prefix='/tmp/planora-muni-v32-canonical-tests-'
...
function Invoke-RetainedV30SnapshotVerifier([string]$Phase){
    ...
    if($row.schema-cne'planora.muni-v32.retained-snapshot-replay.v1'-or$row.status-cne'EXACT_RETAINED_SNAPSHOT_REPLAY'-or$row.phase-cne$Phase-or$row.root-cne$v30SnapshotRoot-or$row.files-ne3146-or$row.directories-ne368-or$row.bytes-ne190900047-or$row.inventory_sha256-cne'b596146131ff2634d55a7f0907497f2fa44ae438174efcb67ee75023ecdb50bb'-or-not[bool]$row.all_nlink_one){throw "Retained v30 snapshot replay semantics rejected: $Phase"};return $row
}
...
if(-not('PlanoraV32NativeFileIdentity'-as[type])){
Add-Type -TypeDefinition @"
using System;
using System.Runtime.InteropServices;
using Microsoft.Win32.SafeHandles;
[StructLayout(LayoutKind.Sequential)]
public struct PlanoraV32FileInformation {
...
public static class PlanoraV32NativeFileIdentity {
 [DllImport("kernel32.dll", SetLastError=true)]
 public static extern bool GetFileInformationByHandle(SafeFileHandle handle, out PlanoraV32FileInformation information);
}
"@
}
function Get-HeldFileIdentity([IO.FileStream]$Stream,[string]$Label){
    $information=New-Object PlanoraV32FileInformation;if(-not[PlanoraV32NativeFileIdentity]::GetFileInformationByHandle($Stream.SafeFileHandle,[ref]$information)){$code=[Runtime.InteropServices.Marshal]::GetLastWin32Error();throw [IO.IOException]::new("$Label GetFileInformationByHandle failed: $code")}
...
function New-ExpectedCombinedPredecessorEvidence{
    ...
    return [ordered]@{schema='planora.muni-v32.complete-v28-v29-v30-v31-predecessor-evidence.v1';status='EXPECTED_UNVALIDATED';contract=$contract;prior_evidence=$null;v30_evidence=$null;runtime=[ordered]@{validation_phase='not_started';validated_pins=@();pass_absence=$null;shared_lock_absent=$null}}
}
...
function Assert-FinalArchivedStaleLockIdentity([object]$ExpectedPin,[string]$Phase,[bool]$RequireSharedLockAbsent){
    ...
    if($RequireSharedLockAbsent-and(Test-Path -LiteralPath $sharedLockPath)){throw "Shared heavy lock unexpectedly present during archived-lock replay: $Phase"};$result['shared_lock_absence_required']=$RequireSharedLockAbsent;$result['shared_lock_absent']=(-not(Test-Path -LiteralPath $sharedLockPath));$result['predecessor_pass_absence']=Assert-V28V29V30V31PassEvidenceAbsent $Phase;return $result
}
function Open-TerminalArchivedStaleLockGuard([object]$ExpectedPin,[string]$Phase){
    ...
    try{$guard=New-Object IO.FileStream($staleArchivePath,[IO.FileMode]::Open,[IO.FileAccess]::Read,[IO.FileShare]::Read);if([IO.Path]::GetFullPath($guard.Name)-cne[IO.Path]::GetFullPath($staleArchivePath)){throw 'Terminal archived-lock guard path rejected'};[void](Assert-HeldStreamBytes $guard $bytes 'dcde7ccade35f6d8a3c9072bfd0ff75bade2c05d479277b42c5ffc2e7ea03b98' 'Terminal archived stale lock guard');[void](Assert-LocalEvidencePin $ExpectedPin);if(Test-Path -LiteralPath $sharedLockPath){throw 'Shared heavy lock unexpectedly present while terminal archive guard held'};$passAbsence=Assert-V28V29V30V31PassEvidenceAbsent ($Phase+'_guard_held');$evidence=[ordered]@{schema='planora.muni-v32.terminal-archived-lock-guard.v1';phase=$Phase;status='IDENTITY_REPLAYED_AND_READ_GUARD_HELD_THROUGH_FINAL_PASS_FLUSH';archive_pin=$ExpectedPin;pre_guard_replay=$preGuardReplay;guard_access='Read';guard_share='Read_only_blocks_write_and_delete';same_handle_bytes_replayed=$true;shared_lock_absent=$true;predecessor_pass_absence=$passAbsence;acquired_at_utc=[DateTime]::UtcNow.ToString('o')};return [pscustomobject]@{Stream=$guard;Evidence=$evidence}}catch{if($null-ne$guard){$guard.Dispose()};throw}
}
...
function Get-ValidatedCombinedPredecessorEvidence([bool]$RequireSharedLockAbsent){
    ...
    if($pins.Count-ne41-or@($pins.path|Sort-Object -Unique).Count-ne41){throw 'v32 combined predecessor pin cardinality rejected'}
    [void](Assert-FinalArchivedStaleLockIdentity $c.v28.archive 'combined_predecessor_validation' $RequireSharedLockAbsent);$pass=Assert-V28V29V30V31PassEvidenceAbsent 'combined_predecessor_validation'
    if($RequireSharedLockAbsent-and(Test-Path -LiteralPath $sharedLockPath)){throw 'Shared lock present before v32 acquisition'}
    $e.status='VALIDATED_EXACT_V28_V29_V30_PREDECESSOR_CUSTODY';$e.prior_evidence=$prior;$e.v30_evidence=[ordered]@{run_id=$c.v30.run_id;authorization=$authorization;claim=$claim;rejection=[ordered]@{pin=$c.v30.artifacts.rejection;status=$rejection.status;failure=$rejection.failure;predecessor_evidence_sha256=$rejection.predecessor_evidence_sha256;snapshot_retained_for_forensics=$rejection.snapshot_retained_for_forensics};nested_predecessor_hashes=[ordered]@{initial_custody=$initialHash;post_rejection=$postHash};lock_evidence=$lockEvidence;lock_release=$release;custody=$custody;staging_inventory_pin=$c.v30.artifacts.staging_inventory;failure=$c.v30.failure};$e.runtime.validation_phase=$(if($RequireSharedLockAbsent){'before_v32_lock_acquisition'}else{'rejection_replay'});$e.runtime.validated_pins=$pins;$e.runtime.pass_absence=$pass;$e.runtime.shared_lock_absent=(-not(Test-Path -LiteralPath $sharedLockPath));return $e
}
...
if($ReadinessPredicateSelfTest){
    ...
    [Console]::Out.WriteLine(([ordered]@{schema='planora.muni-v32.readiness-predicate-self-test.v1';status='PASS';run_id=$runId;runner_sha256=$auth.runner_sha256;authorization_sha256=$auth.authorization_sha256;binding=$binding;canonical_suite_executed=$false;shared_lock_used=$false;wsl_executed=$false}|ConvertTo-Json -Depth 8 -Compress));return
}
if($RetainedV30SnapshotSelfTest){
    if(Test-Path -LiteralPath $sharedLockPath){throw 'Shared heavy lock present before retained v30 snapshot self-test'};$leaf=(Split-Path -Leaf $prefix)+'.';$existing=@(Get-ChildItem -LiteralPath (Split-Path -Parent $prefix) -Force|Where-Object{$_.Name.IndexOf($leaf,[StringComparison]::Ordinal)-eq0});if($existing.Count-ne0){throw 'Fresh v32 artifact namespace is not empty before retained snapshot self-test'}
    ...
    [Console]::Out.WriteLine(([ordered]@{schema='planora.muni-v32.retained-v30-snapshot-self-test.v1';status='PASS';run_id=$runId;runner_sha256=$auth.runner_sha256;authorization_sha256=$auth.authorization_sha256;replay=$replay;canonical_suite_executed=$false;shared_lock_used=$false;claim_created=$false;v32_artifacts_created=$false}|ConvertTo-Json -Depth 8 -Compress));return
}
...
if($LogBridgeSelfTest){
    $auth=Get-AuthorizationState;$name='muni-v32-log-bridge-'+[Guid]::NewGuid().ToString('N');$dir=Join-Path $repo ('tmp\'+$name);$dirWsl=$repoWsl+'/tmp/'+$name
    ...
    try{
        ...
        if($stableReads-lt3){throw 'Fixed bridge did not overlap at least three stable host reads'};[Console]::Out.WriteLine(([ordered]@{schema='planora.muni-v32.log-bridge-self-test.v1';status='PASS';run_id=$runId;runner_sha256=$auth.runner_sha256;legacy_lifetime_open_sharing_failure='REPRODUCED';short_lived_append_stable_reads=$stableReads;rows=$rows.Count;samples=12;duplicates=0;canonical_suite_executed=$false;shared_lock_used=$false}|ConvertTo-Json -Compress));return
...
try{
    ...
    $predecessorEvidence=Get-ValidatedCompletePredecessorEvidence $true;$staleArchivePin=$predecessorEvidence.contract.v28.archive;$predecessorPins=@(Get-CompletePredecessorPinArray $predecessorEvidence);if($predecessorPins.Count-ne61){throw 'Combined predecessor pin cardinality rejected'}
    ...
    $custody=[ordered]@{schema='planora.muni-v32.predecessor-custody.v1';status='EXACT_V28_V29_V30_V31_CUSTODY_VALIDATED_BEFORE_V32_LOCK';run_id=$runId;predecessor_evidence=$predecessorEvidence;predecessor_evidence_sha256=$predecessorEvidenceHash;v31_failure_evidence=$predecessorEvidence.v31_failure_evidence;v31_failure_evidence_sha256=$v31FailureEvidenceHash;archive_identity_replay=(Assert-FinalArchivedStaleLockIdentity $staleArchivePin 'before_predecessor_custody_publication' $true);shared_lock_absent=$true;created_at_utc=[DateTime]::UtcNow.ToString('o')};Write-NewUtf8 $predecessorCustodyFile ($custody|ConvertTo-Json -Depth 70);$predecessorCustodyHash=Get-Sha256 $predecessorCustodyFile
    if(Test-Path -LiteralPath $sharedLockPath){throw 'Shared lock appeared before v32 acquisition'}
    ...
    $retainedV30Terminal=Invoke-RetainedV30SnapshotVerifier 'post_cleanup_while_v32_lock_held_before_final_census';$retainedV30SnapshotTerminalCustody=[ordered]@{schema='planora.muni-v32.retained-v30-snapshot-terminal-custody.v1';status='EXACT_RETAINED_SNAPSHOT_REPLAYED_AFTER_V32_CLEANUP_WHILE_LOCK_HELD';run_id=$runId;initial_custody_sha256=$retainedV30SnapshotCustodyHash;replay=$retainedV30Terminal;created_at_utc=[DateTime]::UtcNow.ToString('o')};Write-NewUtf8 $retainedV30SnapshotTerminalCustodyFile ($retainedV30SnapshotTerminalCustody|ConvertTo-Json -Depth 12);$retainedV30SnapshotTerminalCustodyHash=Get-Sha256 $retainedV30SnapshotTerminalCustodyFile;$retainedV30SnapshotTerminalCustodyPin=Get-LocalEvidencePin $retainedV30SnapshotTerminalCustodyFile
